...
Функция ПолучитьСписокТестов(КонтекстФреймворкаBDD) Экспорт
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|По навигационной ссылке потом можно провести объект шагом <И я выполняю проведение документа по навигационной ссылке 'НавСсылка' (расширение)> или отложить его на пометку удаления: <И я запоминаю навигационную ссылку 'НавСсылка' для удаления (расширение)> копит ссылки, а саму пометку ставит <И я устанавливаю пометку удаления объектам, указанным к удалению (расширение)>.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Код передаётся блоком `""""""bsl` под шагом либо таблицей Gherkin, где каждая строка таблицы становится строкой кода. Результат кода в переменную не возвращается — для этого есть шаг <И Я запоминаю значение выражения 'ВыражениеВстроенногоЯзыка' в переменную 'ИмяПеременной' (Расширение)>.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|### Пример. Вычисление выражения на клиенте тестирования
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Если внутри выражения встречается строка ЗначениеДляВозврата, значит переданное выражение будет само вычислять результат выражения и поместит его в переменную ЗначениеДляВозврата.
		|Подробности в документации к расширению.
		|### Пример. Вычисление выражения на сервере базы клиента тестирования
		|```Gherkin
		|#Простой вариант выражения
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		"### Описание
		|Выводит данные текущего окна в табличный документ. Необходимо, чтобы в клиенте тестирования было подключено расширение VAExtension.
		...
		|### Пример. Вывод данных открытой формы
		|После записи объекта в менеджере тестирования откроется табличный документ с данными формы — по нему видно имена реквизитов, которые можно дальше запрашивать.
		|```Gherkin
		|	И я нажимаю на кнопку с именем 'ФормаЗаписать'
		|	И я вывожу данные текущего окна (Расширение)
		|```
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		"### Описание
		|Позволяет установить значение текущего окна, если оно доступно для записи.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		"### Описание
		|Проверяет, что в базе менеджера тестирования установлено расширение с данным именем
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		"### Описание
		|Проверяет, что в базе менеджера тестирования расширение с данным именем не установлено
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Необходимо обеспечить наличие прав на установку расширения и запуск конфигуратора.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|### Пример. Удаление расширения после сценария
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Флаги перечисляются таблицей Gherkin под шагом, имена — как свойства объекта `РасширениеКонфигурации`: `БезопасныйРежим`, `ЗащитаОтОпасныхДействий`, `ИспользоватьОсновныеРолиДляВсехПользователей`.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|### Пример. Возврат расширению ограничений
		|```Gherkin
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|### Пример. Открытие окна функций для технического специалиста
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		"### Описание
		|Выполняет очистку табличного документа, который находится в отчёте, печатной форме и так далее. Необходимо, чтобы в клиенте тестирования было подключено расширение VAExtension.
		...
		|Табличный документ очищается, отчёт формируется заново, и сценарий ждёт, пока документ снова заполнится.
		|```Gherkin
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Сценарий дожидается первого результата и тут же очищает документ, поэтому второе формирование проверяется на заведомо пустом документе.
		|```Gherkin
		|	И я нажимаю на кнопку 'Сформировать'
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Шаг проверяет размер табличного документа, и если он больше нуля по горизонтали или вертикали, считается, что документ заполнен данными.
		|Перед формированием табличного документа стоит выполнить <И я очищаю табличный документ 'ИмяРеквизита' (расширение)>, иначе шаг увидит данные прошлого формирования.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Проверяется свойство `ТолькоПросмотр` **всей формы**, а не отдельного поля — для отдельного поля есть проверка <И элемент 'Заголовок элемента' доступен только для просмотра>. Типовое применение — убедиться, что проведённый документ или закрытый период не дают себя править.
		|### Пример. Проведённый документ открыт только на просмотр
		|Ссылка сохраняется, пока форма ещё открыта, затем документ проводится, открывается заново по ссылке — и сценарий подтверждает, что форма перешла в режим только просмотра.
		|```Gherkin
		|	И я сохраняю ссылку на объект из текущего окна в переменную 'СсылкаНаДокумент'
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Нумерация сквозная по документу, но считаются только элементы с `type=button`: обычный `<button>` и кнопку отправки формы шаг не видит. Как и с гиперссылками, поиск по номеру ломается при изменении вёрстки — надёжнее <И у поля с именем 'ИмяЭлемента' я нажимаю кнопку HTML по имени 'ИмяКнопкиHTML' (расширение)>.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|### Пример. Ветвление по признаку проведения
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Обработка открывается сразу: ни диалога выбора файла, ни вопроса о безопасности сценарий не увидит.
		|```Gherkin
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Нужен как мост между обычными шагами и библиотекой UI Automation: сначала по заголовку находят элемент формы 1С и получают его ID, а дальше этим ID оперируют шаги UI Automation. Если у элемента нет заголовка, берите шаг <И я запоминаю элемент формы клиента тестирования с именем 'ИмяЭлемента' в переменную 'ИмяПеременной' UI Automation (расширение)>. Оба шага на время подменяют подсказку элемента, поэтому у элемента без свойства «Подсказка» они не сработают.
		|### Пример. ID элемента по заголовку на форме
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
			...
			"### Описание
			|Генерирует код маркировки и сохраняет его в переменную.
			|Типы КМ: 5, 23, 30, 31 (информацию о типах смотрите в документации API СУЗ ОБЛАКО)
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
			...
			|Передача кода идёт через буфер обмена, буфер обмена не очищается
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
			...
			|Этот шаг открывает всю группу шагов ""через файл события"": пока мониторинг не запущен, они работать не будут. Каталог создаёт сам шаг, а внутри него — подкаталог с PID клиента тестирования: через него и идёт обмен файлами событий.
			|- Необходимо, чтобы в клиенте тестирования было подключено расширение VAExtension 1.04 и выше.
			|- Чтение каталога с файлами происходит каждую секунду, что может вызвать дополнительную нагрузку на систему.
			|- Мониторинг обязательно останавливать шагом <И я Останавливаю мониторинг каталога ""C:\temp"" внешних событий(Расширение)>: сам он не выключится и продолжит опрашивать каталог после окончания сценария.
			|### Пример. Запуск и остановка мониторинга
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
			...
			|можно использовать тег ""\x_GS"" для передачи символа FNC1(29)
			|Используется эмуляция вызова 1С:Библиотека подключаемого оборудования
			|Отличие от <И я эмулирую сканирование штрихкода БПО '4670003110011' через буфер обмена> — событие передаётся файлом, а не буфером обмена, поэтому шаг работает при открытом модальном окне и не портит буфер.
			|- Необходимо, чтобы в клиенте тестирования было подключено расширение VAExtension 1.04 и выше.
			|- Код выполняется только через вызов модуля приложения.
			|- Требуется предварительно запущенный мониторинг каталога событий: <И я запускаю мониторинг каталога ""C:\temp"" для внешних событий(Расширение)>. В конце сценария мониторинг обязательно останавливать шагом <И я Останавливаю мониторинг каталога ""C:\temp"" внешних событий(Расширение)> — сам он не выключится.
			|- Чтение каталога происходит каждую секунду, что может вызвать дополнительную нагрузку на систему. Во время отладки следите за порядком выполнения шагов.
			|### Пример. Сканирование с получением результата
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
			...
			"### Описание
			|Выполняет произвольный код на стороне клиента в тестируемом приложении.
			|В отличие от варианта через буфер обмена, работает при открытом модальном окне и не занимает буфер.
			|- Необходимо, чтобы в клиенте тестирования было подключено расширение VAExtension 1.04 и выше.
			|- Код выполняется только через вызов модуля приложения.
			|- Требуется предварительно запущенный мониторинг каталога событий: <И я запускаю мониторинг каталога ""C:\temp"" для внешних событий(Расширение)>. В конце сценария мониторинг обязательно останавливать шагом <И я Останавливаю мониторинг каталога ""C:\temp"" внешних событий(Расширение)> — сам он не выключится.
			|- Чтение каталога происходит каждую секунду, что может вызвать дополнительную нагрузку на систему. Во время отладки следите за порядком выполнения шагов.
			|- Результат шага забирается отдельным шагом <И я ожидаю ""10"" секунд результат обработки последнего события через файл и запоминаю результат в переменную ""ИмяПеременной""(Расширение)>: в переменную попадёт значение переменной ""Результат"" из тестируемого приложения.
			|### Пример. Клиентский код с возвратом значения
			|Результат кода не возвращается самим шагом — его забирает следующий шаг.
			|```Gherkin
			|	И я выполняю код встроенного языка на клиенте через файл события (Расширение)
			|		""""""bsl
			|			Результат = ""Код отработал"";
			|		""""""
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
			...
			|- Необходимо, чтобы в клиенте тестирования было подключено расширение VAExtension 1.04 и выше.
			|- Код выполняется только через вызов модуля приложения.
			|- Работает при открытом модальном окне.
			|- Требуется предварительно запущенный мониторинг каталога событий: <И я запускаю мониторинг каталога ""C:\temp"" для внешних событий(Расширение)>. В конце сценария мониторинг обязательно останавливать шагом <И я Останавливаю мониторинг каталога ""C:\temp"" внешних событий(Расширение)> — сам он не выключится.
			|- Чтение каталога происходит каждую секунду, что может вызвать дополнительную нагрузку на систему. Во время отладки следите за порядком выполнения шагов.
			|- Результат шага забирается отдельным шагом <И я ожидаю ""10"" секунд результат обработки последнего события через файл и запоминаю результат в переменную ""ИмяПеременной""(Расширение)>: в переменную попадёт значение переменной ""Результат"" из тестируемого приложения.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
			...
			|- Чтение каталога происходит каждую секунду, что может вызвать дополнительную нагрузку на систему. Во время отладки следите за порядком выполнения шагов.
			|- Результат шага забирается отдельным шагом <И я ожидаю ""10"" секунд результат обработки последнего события через файл и запоминаю результат в переменную ""ИмяПеременной""(Расширение)>: в переменную попадёт значение переменной ""Результат"" из тестируемого приложения.
			|### Пример. Чтение данных, закрытых от текущего пользователя
			|```Gherkin
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
			...
			|Самый общий шаг этой группы: им отправляют любое внешнее событие, а не только сканер. Три параметра — источник, событие и данные — это ровно те, что приходят в обработчик `ОбработкаВнешнегоСобытия` модуля приложения.
			|- Необходимо, чтобы в клиенте тестирования было подключено расширение VAExtension 1.04 и выше.
			|- Код выполняется только через вызов модуля приложения.
			|- Работает при открытом модальном окне.
			|- Требуется предварительно запущенный мониторинг каталога событий: <И я запускаю мониторинг каталога ""C:\temp"" для внешних событий(Расширение)>. В конце сценария мониторинг обязательно останавливать шагом <И я Останавливаю мониторинг каталога ""C:\temp"" внешних событий(Расширение)> — сам он не выключится.
			|- Чтение каталога происходит каждую секунду, что может вызвать дополнительную нагрузку на систему. Во время отладки следите за порядком выполнения шагов.
			|- Результат шага забирается отдельным шагом <И я ожидаю ""10"" секунд результат обработки последнего события через файл и запоминаю результат в переменную ""ИмяПеременной""(Расширение)>: в переменную попадёт строка «Событие было обработано», а если обработчик упал — текст ошибки.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
			...
			|Код вернул строку через `Результат`, и она оказывается в переменной. В окно сообщений попадёт `Код отработал`.
			|```Gherkin
			|	И я выполняю код встроенного языка на клиенте через файл события (Расширение)
			|		""""""bsl
			|			Результат = ""Код отработал"";
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Ссылка снимается с открытой формы, документ проводится без её открытия, а в окно сообщений попадает признак проведения.
		|```Gherkin
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Документ списывает больше, чем есть на складе, поэтому провестись он не должен, а текст отказа сценарий сверяет целиком.
		|```Gherkin
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|### Пример. Отмена проведения в конце сценария
		|Документ распроводится, после чего сценарий забирает его состояние и выводит его — в окне сообщений будет `Нет`.
		|```Gherkin
		|	И я отменяю проведение документа по навигационной ссылке '$НавСсылка$' (расширение)
		|	И я получаю состояние объекта по навигационной ссылке '$НавСсылка$' (расширение)
		|	И я вывожу значение переменной 'Проведен'
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Ссылки не перечисляют руками: шаг сам находит их в тексте сценария. Обратите внимание, что просматривается **весь** сценарий, включая ещё не выполненные шаги.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Форма отчёта не открывается, поэтому проверка идёт заметно быстрее, чем через интерфейс.
		|Имя отчёта указывается как в конфигураторе. А вот имя варианта — это **Наименование** из справочника ""Варианты отчетов"", то есть подпись варианта, как её видит пользователь в панели выбора, а не технический ключ варианта из конфигуратора. Для многих типовых отчётов эти строки совпадают, но не всегда.
		|Эталон — макет табличного документа: сначала он ищется среди макетов обработки с библиотекой шагов, а если там не нашёлся — в каталоге проекта. Сравнение точное; если в отчёт попадают даты или номера, берите <И вариант отчета 'ИмяОтчета' 'ИмяВариантаОтчета' равен макету 'ИмяЭталона' по шаблону (расширение)>.
		|### Пример. Сверка отчёта с эталоном
		|```Gherkin
		|	И вариант отчета 'ОстаткиТоваров' 'Остатки товаров' равен макету 'ЭталонОстатков' (расширение)
		|```
		|",
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|### Пример. Сверка отчёта с датой формирования в шапке
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ЯЗапоминаюНавигационнуюСсылкуТекущегоОкнаДляУдаленияРасширение()",
		...
		|### Пример. Пометка на удаление созданного объекта
		|Ссылку запоминают сразу после записи, а в конце сценария по накопленным ссылкам ставится пометка удаления.
		|```Gherkin
		|	И я нажимаю на кнопку с именем 'ФормаЗаписать'
		|	И я запоминаю навигационную ссылку текущего окна для удаления (расширение)
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ЯЗапоминаюНавигационнуюСсылкуДляУдаленияРасширение(Парам01)",
		...
		|### Пример. Пометка на удаление по сохранённой ранее ссылке
		|```Gherkin
		|	И я сохраняю навигационную ссылку текущего окна в переменную 'НавСсылка' (Расширение)
		|	И я закрываю текущее окно
		|	И я запоминаю навигационную ссылку '$НавСсылка$' для удаления (расширение)
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ЯУстанавливаюПометкуУдаленияОбъектамУказаннымКУдалениюРасширение()",
		...
		|### Пример. Пометка на удаление в конце сценария
		|```Gherkin
		|	И я запоминаю навигационную ссылку текущего окна для удаления (расширение)
		|	И я устанавливаю пометку удаления объектам, указанным к удалению (расширение)
		|```
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ЯПолучаюДанныеОПодключенныхРасширенияхКлиентаТестированияРасширение()",
		...
		"### Описание
		|Получает данные об установленных расширениях и создаёт для каждого расширения переменную:
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Свойство переключается в значение «При изменении значения» у полей ввода текущего окна; флажки, переключатели, поля надписи и поля текстового документа шаг пропускает. Работает только с окнами форм справочников и документов: если в заголовке окна нет скобок и нет « от », шаг молча ничего не делает. Если известно конкретное проблемное поле, аккуратнее взять <И я отключаю обновление текста редактирования поля с именем 'ИмяПоля' в текущем окне (расширение)>.
		|Симптом, ради которого шаг нужен: сценарий ввёл значение, следующая проверка видит старое, а руками всё вводится нормально.
		|### Пример. Отключение обновления на всей форме
		|Свойство переключается сразу после открытия формы, до ввода данных.
		|```Gherkin
		|	И я отключаю обновление текста редактирования полей в текущем окне (расширение)
		|	И в поле с именем 'Наименование' я ввожу текст '111'
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Точечный вариант шага <И я отключаю обновление текста редактирования полей в текущем окне (расширение)>: трогает поля с указанным заголовком, а не всю форму.
		|### Пример. Отключение обновления у одного поля
		|```Gherkin
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|### Пример. Отключение обновления у поля по имени
		|```Gherkin
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Один из трёх шагов группы, рядом с <Если это интерфейс Такси (расширение) Тогда> и <Если это интерфейс82 (расширение) Тогда>. Нужны там, где в разных вариантах интерфейса отличаются расположение элементов и способ навигации.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|В интерфейсе Такси раздел открывают шагом работы с командным интерфейсом, в 8.2 — через панель разделов.
		|```Gherkin
		|	Если это интерфейс Такси (расширение) Тогда
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Шаг не условие: если база серверная, сценарий упадёт. Нужен, чтобы сценарии, рассчитанные только на файловую базу, не запускались в серверной, — например, те, что обращаются к файлу `1Cv8.1CD` или пользуются монопольным режимом. Зеркальный шаг — <И клиент тестирования запущен в серверной базе (расширение)>.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Зеркальный шаг — <И клиент тестирования запущен в файловой базе (расширение)>; он так же не является условием: на файловой базе сценарий упадёт. Нужен для проверок, которые имеют смысл только в клиент-серверном варианте: фоновые задания, регламентные задания, блокировки.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|### Пример. Открытие объекта в одной базе с менеджером тестирования
		|Менеджер тестирования и клиент тестирования работают с одной базой, поэтому ссылку передают как есть.
		|```Gherkin
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|### Пример. Открытие объекта, когда базы разные
		|Базы разные, поэтому ссылку передают строковым представлением.
		|```Gherkin
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|### Пример. Пометка на удаление документов по комментарию
		|```Gherkin
