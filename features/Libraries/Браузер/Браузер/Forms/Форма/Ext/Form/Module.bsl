...
Функция ПолучитьСписокТестов(КонтекстФреймворкаBDD) Экспорт
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты, "ЯПоказываюПодсказкуEnjoyHintПоКоординатамВБраузере(Парам01,Парам02,ТабПарам)",
		...
		|Если второй параметр не указан, то left, top, width, height берутся из переданной таблицы.
		|Такую переменную можно создать программно, а можно получить шагом <И я определяю координаты картинки """"ИмяКартинки"""" как """"ИмяПеременной""""> из библиотеки SikuliX.
		|Учтите: одни и те же координаты дадут одну и ту же область только при браузере, открытом на полный экран (F11 для Chrome). Иначе координаты придётся пересчитывать под размер окна
		|noFreeze - значение true позволяет избежать лагов анимации, но на время выполнения эффекта будет невозможно выполнение других шагов.
		|Дополнительная информация по EnjoyHint находится тут: https://github.com/xbsoftware/enjoyhint/blob/master/README.md
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты, "ЯПоказываюПодсказкуEnjoyHintУОбластиТаблицыВБраузере(Текст, ИмяТаблицы, ТабПарам)",
		...
		|Если указать только rowNumber - область охватит одну указанную строку.
		...
		|noFreeze - значение true позволяет избежать лагов анимации, но на время выполнения эффекта будет невозможно выполнение других шагов.
		|Дополнительная информация по EnjoyHint находится тут: https://github.com/xbsoftware/enjoyhint/blob/master/README.md
		|### Пример. Подсветить диапазон колонок в двух строках таблицы
		|```Gherkin
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты, "ЯПоказываюСериюПодсказокEnjoyHintВБраузере(ТабПарам1, ТабПарам2, ТабПарам3, ТабПарам4, ТабПарам5, ТабПарам6, ТабПарам7, ТабПарам8,  ТабПарам9, ТабПарам10)",
		...
		|### Пример. Показать три подсказки подряд: поля, область страницы, ячейки таблицы
		|```Gherkin
		|	И я показываю серию подсказок EnjoyHint в браузере
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты, "ЯНакладываюЭффектРазмытияВБраузере(ТабПарам)",
		...
		|		|'timeout'|'3000'|",
		"### Описание
		|Накладывает размытие на одну или несколько областей (максимум 10 шт.)
		...
		|Для типа coords:
		|Значения левого-верхнего угла, и размер области: left, top, width, height.
		|### Пример. Размыть колонку с суммами и поле контрагента
		|```Gherkin
		|	И я накладываю эффект размытия в браузере
		|		| 'blur'    | '3'          |
		|		| 'type'    | 'elems'      |
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты, "ЯРисуюСтрелкуОтКВБраузере(Парам01,Парам02,ТабПарам)",
		...
		|`startSocket` и `endSocket` — с какой стороны элемента выходит и куда входит стрелка: `top`, `bottom`, `left`, `right`, `auto`. В шаблоне шага подставлены `up` и `down` — таких значений LeaderLine не знает, замените их при вставке. `startSocketGravity` и `endSocketGravity` — пара чисел, задающих изгиб: чем больше значение, тем сильнее стрелка отходит в эту сторону, прежде чем повернуть.
		...
		|	И пауза 8
		|```
		|### Пример. Провести стрелку от надписи к полю формы
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты, "ЯРисуюРамкуВокругЯчеекТаблицыВБраузере(ИмяТаблицы, ТабПарам)",
		...
		|Если указать только rowNumber - рамка охватит одну указанную строку.
		|Если не указать ни один из этих параметров - рамка охватит все строки
		|То же самое справедливо и для отбора по колонкам.
		|Для колонок, вместо заголовка можно указывать порядковый номер (columnNumber, columnNumberEnd)
		|### Пример. Обвести диапазон колонок в двух строках
		|```Gherkin
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты, "ЯРисуюРамкуВокругЭлементаПанелиРазделовВБраузере(Парам01,ТабПарам)",
		...
		|		|'timeout'|'5000'|",
		"### Описание
		|Рисует рамку вокруг раздела панели разделов в браузере. Раздел ищется по заголовку.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты, "ЯРисуюРамкуВокругЭлементаПанелиФункцийВБраузере(Парам01,ТабПарам)",
		...
		|		|'timeout'|'5000'|",
		"### Описание
		|Рисует рамку вокруг команды панели функций в браузере. Команда ищется по заголовку.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты, "ЯПоказываюТекстНадПолемВБраузере(Парам01,Парам02,ТабПарам)",
		...
		|`offsetup` — на сколько пикселей поднять надпись над полем: чем больше значение, тем выше. Нужен, когда надпись перекрывает соседний элемент.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты, "ЯПоказываюЗаставкуВБраузере(Парам01,ТабПарам)",
		...
		|Единственный шаг библиотеки с явной проверкой типа клиента: если текущий клиент тестирования не Web, шаг молча ничего не делает — ни заставки, ни ошибки. Сценарий при этом не падает, поэтому пропажу титра в готовом ролике легко не связать с типом клиента тестирования.
		...
		|### Пример. Показать титр между частями инструкции
		|```Gherkin
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты, "ЯПоказываюНажатиеМышиВБраузере(ТабПарам)",
		...
		|`radius` — радиус кругов в пикселях, `width` — толщина линии, `duration` — длительность анимации; если параметр не задан, берётся значение из настроек VA, `transparency` — прозрачность от 0 (непрозрачно) до 255.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты, "ЯЗакрываюВсеВкладкиБраузера()",
		...
		|Закрытие всех вкладок перед началом работы или после неё: браузер должен остаться без лишних вкладок, чтобы VA открыла клиента тестирования в чистом окне. Закрываются все вкладки без разбора, включая ту, в которой открыт веб-клиент.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		|Заголовок вкладки — то, что написано на самой вкладке, а не адрес страницы. Звёздочка заменяет любую последовательность символов, поэтому по маске закрывается вкладка с заголовком, который меняется от прогона к прогону.
		|### Пример. Закрыть вкладку, оставшуюся после выхода из сеанса
		|```Gherkin
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты, "ЯАктивизируюБраузерИУстанавливаюПолноэкранныйРежим()",
		...
		|То же, что <И я активизирую текущее окно браузера>, плюс переход в полноэкранный режим (F11): пропадают адресная строка, вкладки и панель задач. Для видеоинструкции это важно вдвойне — в кадре остаётся только интерфейс 1С, без посторонних окон и панелей браузера.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты, "ЯПоказываюПодсказкуDriverJsУЭлементаВБраузере(Парам01,Парам02,ТабПарам)",
		...
		|Элемент ищется **по имени**. `position` — с какой стороны от элемента показать карточку (`top`, `bottom`, `left`, `right`), `padding` — отступ выреза от границ элемента в пикселях, `opacity` — плотность затемнения от 0 до 1, `animate` — плавно ли перемещать вырез, `timeout` — через сколько миллисекунд снять подсветку.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты, "ЯПоказываюАнимированныйБлокТекстаВБраузере(ТабПарам, ТабПарам1, ТабПарам2, ТабПарам3, ТабПарам4)",
		...
		|Оформление задаётся через CSS: имя параметра пишется как в CSS с префиксом `css-`, значение — как в CSS. Так, `css-background-color` со значением `red` даст красную подложку, а `css-bottom` со значением `10%` прижмёт блок к низу экрана.
		|### Пример. Показать титры внизу экрана
		|```Gherkin
		|	И я показываю анимированный блок текста в браузере
		|		| 'effect'             | 'fold'  |
		|		| 'duration'           | '2000'  |
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты, "ЯРисуюРамкуПоКоординатамВБраузере(ТабПарам)",
		...
		|Вариант для случая, когда обвести нужно не элемент формы, а произвольный участок экрана: часть картинки, область без собственного имени, кусок отчёта. Все размеры в пикселях, отсчёт от левого верхнего угла окна браузера.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты, "ЯПоказываюАнимациюЗагрузкиВБраузере()",
		...
		|Крутящийся индикатор поверх страницы. Именно имитация: ничего не загружается и не ждётся, шаг нужен для видеоинструкции — им заполняют место вырезанной паузы, чтобы у зрителя не возникло ощущения обрыва. Анимация идёт около пяти секунд и сценарий не задерживает — чтобы она успела доиграть, следом ставят <И пауза 5>.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты, "ЯОткрываюПодсказкуПоляСИменемВБраузере(ИмяПоля)",
		...
		|Речь о всплывающей подсказке самой конфигурации — той, что открывается по кнопке со знаком вопроса рядом с полем (свойство «Подсказка» в конфигураторе). Это не эффект видеоинструкции: шаг показывает штатную подсказку конфигурации, а не рисует что-то поверх страницы.
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты, "ЯНажимаюНаHTMLЭлементВБраузере(Элемент)",
		...
		|Апострофы внутри выражения экранируются обратной косой чертой: `\'`. Перед решёткой пишут три косых — `\\\#`: одну снимает парсер фич, вторую — JavaScript, а последняя нужна CSS-селектору, в который `elem` подставляет имя элемента.
