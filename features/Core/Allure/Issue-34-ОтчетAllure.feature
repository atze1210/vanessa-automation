...
Сценарий: В json Allure поле historyId должно заполняться всегда, даже когда фича не находится в каталоге проекта
	И я перехожу к закладке с именем "ГруппаКаталогиБиблиотек"
		
	Когда В поле с именем "КаталогФичСлужебный" я указываю путь к служебной фиче "Allure/ФичаДляПроверкиОтчетаAllure46"
	...
	И в поле с именем 'КаталогПроекта' я ввожу текст "c:\temp\ВременныйКаталогПроекта"
	...
	Тогда таблица "ДеревоТестов" стала равной:
		| 'Наименование'                          | 'Статус'  |
		| 'ФичаДляПроверкиОтчетаAllure46.feature' | ''        |
		| 'ФичаДляПроверкиОтчетаAllure46'         | ''        |
		| 'СценарийФичаДляПроверкиОтчетаAllure46' | 'Success' |
		| 'И пауза 1'                             | 'Success' |
	И в каталоге аллюр появился 1 файл json
	И я читаю json файл "$НайденноеИмяФайлаjson$" в переменную "ДанныеJson"
	Тогда выражение внутреннего языка 'Контекст.ДанныеJson.historyId' имеет значение 'ФичаДляПроверкиОтчетаAllure46.СценарийФичаДляПроверкиОтчетаAllure46'


//https://github.com/beefree86/vanessa-automation-VAZON/issues/3
Сценарий: Проверка отчета Allure когда добавляются произвольные параметры
	Когда В поле с именем "КаталогФичСлужебный" я указываю путь к служебной фиче "Allure/ФичаДляПроверкиОтчетаAllure45"
	И я перехожу к закладке с именем "ГруппаНастройки"
	И я перехожу к закладке с именем "СтраницаОтчетыОЗапуске"
	...
	И json файл "$НайденноеИмяФайлаjson$" содержит значения в узле "parameters"
        | 'ПараметрСценария' | 'МойТестовыйпараметр' |
        | 'ИмяБазы'          | 'МояТестоваяБаза'     |
        | 'ИмяСервера'       | 'МойтестовыйСервер'   |

	И я читаю json файл "$НайденноеИмяФайлаjson$" в переменную "ДанныеJson"
	// Контроль поля historyId
	Тогда выражение внутреннего языка 'Контекст.ДанныеJson.historyId' имеет значение 'features/support/templates/allure/фичадляпроверкиотчетаallure45.feature_выполнение первого простого сценария. тестовая фича, проверяющая добавление параметров в отчет allure_мойтестовыйпараметр_моятестоваябаза_мойтестовыйсервер_fb7b36bf408e7b018503c7544cae56aa'

// Проверка настройки "Объединять примеры структуры сценария в Allure": включена — все примеры одной структуры выгружаются одним тест-кейсом
Сценарий: Объединение примеров структуры сценария в один тест кейс Allure
	Когда В поле с именем "КаталогФичСлужебный" я указываю путь к служебной фиче "Allure/ФичаДляПроверкиОтчетаAllure47"
	И я перехожу к закладке с именем "ГруппаНастройки"
	И я перехожу к закладке с именем "СтраницаОтчетыОЗапуске"
	
	И я устанавливаю флаг с именем 'ДелатьОтчетВФорматеАллюр'
	И в поле каталог отчета аллюр я указываю путь к относительному каталогу "tools\Allure"
	И я устанавливаю флаг с именем 'ОбъединятьПримерыСтруктурыСценарияВAllure'
	...
	// У всех примеров одной структуры fullName и testCaseId одинаковые, отличия — в historyId (номер примера и значения параметров)
	И для каждого файла "ТекФайл" из каталога "$КаталогВыгрузкиAllure$"
		И я читаю json файл "$ТекФайл$" в переменную "ДанныеJSON"
		И выражение внутреннего языка '$ДанныеJSON$.fullName = "features/support/templates/allure/фичадляпроверкиотчетаallure47.feature_объединение примеров структуры в один тест кейс allure"' Истинно
		И выражение внутреннего языка '$ДанныеJSON$.testCaseId = "3c8d6c11c548b76992ff2a3d9baf0f3c"' Истинно
		И выражение внутреннего языка 'Найти($ДанныеJSON$.historyId, "№1") > 0 ИЛИ Найти($ДанныеJSON$.historyId, "№2") > 0' Истинно
		И выражение внутреннего языка '$ДанныеJSON$.historyId <> $ДанныеJSON$.fullName' Истинно
		И файл "$ТекФайл$" содержит строки
			| '"name": "ПервыйПараметр"' |
			| '"name": "ВторойПараметр"'  |
			| '"name": "ПараметрШага"'    |
		Если 'Найти($ДанныеJSON$.name, "№1") > 0' Тогда
			И файл "$ТекФайл$" содержит строки
				| '"value": "Значение1"' |
		ИначеЕсли 'Найти($ДанныеJSON$.name, "№2") > 0' Тогда
			И файл "$ТекФайл$" содержит строки
				| '"value": "Значение22"' |

// Настройка выключена — прежний формат: каждый пример отдельный тест-кейс
Сценарий: Не объединять примеры структуры сценария в Allure (прежний формат выгрузки)
	Когда В поле с именем "КаталогФичСлужебный" я указываю путь к служебной фиче "Allure/ФичаДляПроверкиОтчетаAllure47"
	И я перехожу к закладке с именем "ГруппаНастройки"
	И я перехожу к закладке с именем "СтраницаОтчетыОЗапуске"
	И я снимаю флаг с именем 'ОбъединятьПримерыСтруктурыСценарияВAllure'
	...
	// Прежний формат: fullName разных примеров различаются (содержат №1/№2), имя без подстановки параметров
	И для каждого файла "ТекФайл" из каталога "$КаталогВыгрузкиAllure$"
		И я читаю json файл "$ТекФайл$" в переменную "ДанныеJSON"
		И выражение внутреннего языка 'Найти($ДанныеJSON$.fullName, "№1") > 0 ИЛИ Найти($ДанныеJSON$.fullName, "№2") > 0' Истинно
		И выражение внутреннего языка 'Найти($ДанныеJSON$.name, " | ") = 0' Истинно

Сценарий: Прикрепление скриншота, когда используется VanessaExt. Одно окно. Весь экран. Плюс дополнительные процессы.
...
Сценарий: Проверка остановки выполнения сценария, когда шаг остановки последний в сценарии
	И я перехожу к закладке с именем "ГруппаКаталогиБиблиотек"
...
Сценарий: Завершение выполнения сценария с помощью шага и формирования отчета Allure. Старый парсер.
	И я перехожу к закладке с именем "ГруппаКаталогиБиблиотек"
...
Сценарий: Завершение выполнения сценария с помощью шага и формирования отчета Allure. Новый парсер.
	И я перехожу к закладке с именем "ГруппаКаталогиБиблиотек"
