...
Сценарий: Основной сценарий проверки работы шагов Gherkin из поставляемой библиотеки и проверка генерации шагов.
	...
	И я выполняю команду системы "echo 111" и сохраняю результат в переменную "echo2"
	И я выполняю команду системы 'echo 111%% 222' и сохраняю результат в переменную "echo3"
	Если 'Найти(Контекст.echo1, "%") > 0' Тогда
		Тогда я вызываю исключение 'Ошибка в значении переменной echo1/ Найдена подсрока "%"'
	ИначеЕсли 'Найти(Контекст.echo1, "appdata") > 0' Тогда
		Тогда я вызываю исключение 'Ошибка в значении переменной echo1/ Найдена подсрока "appdata"'
	И переменная "echo2" имеет значение "111"
	И переменная "echo3" имеет значение "111% 222"
...
#Область Создание элементов шапки	
	...
	//появилось окно с заголовком из таблицы: подходит вторая строка таблицы
	Затем Я запоминаю значение выражения "0" в переменную "ПроверкаОкнаТаблица"
	Если появилось окно с заголовком из таблицы в течение 10 секунд Тогда
		| "НетТакогоОкна" |
		| "Справочник1"   |
			Затем Я запоминаю значение выражения "1" в переменную "ПроверкаОкнаТаблица"
	Иначе
			Затем Я запоминаю значение выражения "2" в переменную "ПроверкаОкнаТаблица"
	Тогда переменная "ПроверкаОкнаТаблица" имеет значение 1
	
	//окно с заголовком из таблицы не появилось: должна сработать ветка Иначе
	Затем Я запоминаю значение выражения "0" в переменную "ПроверкаОкнаНетТаблицы"
	Если появилось окно с заголовком из таблицы в течение 2 секунд Тогда
		| "НетТакогоОкна1" |
		| "НетТакогоОкна2" |
			Затем Я запоминаю значение выражения "1" в переменную "ПроверкаОкнаНетТаблицы"
	Иначе
			Затем Я запоминаю значение выражения "2" в переменную "ПроверкаОкнаНетТаблицы"
	Тогда переменная "ПроверкаОкнаНетТаблицы" имеет значение 2
	...
	И я запоминаю текущее окно как "ТекущееОкноДоОткрытияСЗапретомНаПустоеЗначение" с запретом на пустое значение
	Тогда переменная "ТекущееОкноДоОткрытияСЗапретомНаПустоеЗначение" имеет значение "$ТекущееОкноДоОткрытия$"
...
#Область Проверка значений элементов формы
	...
	И     элемент формы с именем "РеквизитБулево" стал равен "Да"
	И     элемент формы с именем "РеквизитПереключатель" стал равен 'Первое значение'

	И я активизирую поле с именем 'РеквизитПереключатель'
	
	И я запоминаю строку "Нет" в переменную "УсловиеИстинно"
	Если элемент формы с именем "РеквизитПереключатель" имеет значение "1" тогда
		И я запоминаю строку "Да" в переменную "УсловиеИстинно"
	Тогда переменная "УсловиеИстинно" имеет значение "Да"
	...
	// Также проверка на старый формат значения
	Тогда элемент формы с именем 'РеквизитПереключательВысотаСтроки2' стал равен "1"

	* Проверка, что шаги таблица не содержит корректно работают, когда в таблице нет строк
		Тогда таблица 'ТабличнаяЧасть2' не содержит строки 
			| 'N' |
			| '1' |
		Тогда таблица 'ТабличнаяЧасть2' не содержит строки по шаблону
			| 'N' |
			| '1' |
		
	* Проверка, что шаги таблица содержит корректно работают, когда в таблице нет строк
		//https://github.com/Pr-Mex/vanessa-automation/issues/2440
		
		И Я запоминаю значение выражения '0' в переменную "БылоИсключение"
		Попытка	
			Тогда таблица 'ТабличнаяЧасть2' содержит строки 
				| 'N' |
				| '1' |
		Исключение
			И Я запоминаю значение выражения '1' в переменную "БылоИсключение"
		Тогда переменная "БылоИсключение" имеет значение 1
		
		И Я запоминаю значение выражения '0' в переменную "БылоИсключение"
		Попытка	
			Тогда таблица 'ТабличнаяЧасть2' содержит строки по шаблону
				| 'N' |
				| '1' |
		Исключение
			И Я запоминаю значение выражения '1' в переменную "БылоИсключение"
...
#Область Проверка созданного кода Gherkin по изменениям элементов формы
	...
	Тогда В реквизите Vanessa-Behavior "СгенерированныйСценарий" будет содержаться строка 'элемент формы с именем "РеквизитДата" стал равен \'08.04.1981\''
	...
	Тогда В реквизите Vanessa-Behavior "СгенерированныйСценарий" будет содержаться строка 'элемент формы с именем "РеквизитПереключатель" стал равен \"Первое значение\"'
