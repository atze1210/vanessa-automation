...
Функция СоздатьВидеоНужнойДлительностиИзКартинки(ПараметрыВидео, КартинкаТекст, ДлительностьВидео, ДелатьFadeIn = Истина, ДелатьFadeOut = Истина)
	...
	Если ДелатьFadeOut Тогда
		...
		//	+ """ -y -qscale 1 -vf fade=out:" + Формат((ДлительностьВидео - 1) * 30 - 1, "ЧГ=; ЧН=0") +":30 """ + КартинкаВидеоПереход2 + """";
		Команда = """" + ПараметрыВидео.ЗаписьВидеоКомандаFfmpeg + """ -i """ + КартинкаВидеоПереход1 
			+ """ -y -qscale 1 -vf ""fade=out:st=" + XMLСтрока(ДлительностьВидео - 1) + ":d=1,tpad=stop_mode=add:stop_duration=0.1:color=black"" """ + КартинкаВидеоПереход2 + """";
		Команда = СтрокаОтключенияВыводаКонсоли(Команда, ПараметрыВидео);
		Ванесса.Отладка(Команда);
		ВыполнитьКоманду(ПараметрыВидео, Команда);
...
Функция СоздатьФайлВидеоИзТекста(ПараметрыВидео, Текст, MP3, ФинальноеИмяФайла)
	...
	Если Mp3 <> Неопределено Тогда
		...
		ФайлТишины2 = СоздатьИмяФайлаТишины(ПараметрыВидео, 1, ФинальноеИмяФайла + "_silence_2");  //+++ ЦВ_ПР 27.10.2025 Уменьшить длительность тишины  вместо 10 сделать 1 сек  (может быть потом через параметр задавать?)
...
Процедура СоздатьIntro(ПараметрыВидео)
	...
	Если ПараметрыВидео.ЗаписьВидеоДелатьНаложениеTTS Тогда
		...
		КонецЕсли;	 
	Иначе	
		ФайлMp3         = СоздатьИмяФайлаТишины(ПараметрыВидео, 1, "intro_silence_0");  //+++ ЦВ_ПР 27.10.2025 Уменьшить длительность тишины  вместо 10 сделать 1 сек  (может быть потом через параметр задавать?)
	КонецЕсли;	 
	...
	ФайлТишины2 = СоздатьИмяФайлаТишины(ПараметрыВидео, 4, "intro_silence_2");  //+++ ЦВ_ПР 27.10.2025 Уменьшить длительность тишины  вместо 20 сделать 4 сек  (может быть потом через параметр задавать?)
...
Процедура СоздатьOutro(ПараметрыВидео)
	...
	ФайлТишины = СоздатьИмяФайлаТишины(ПараметрыВидео, 1, "outro_silence");  //+++ ЦВ_ПР 27.10.2025 Уменьшить длительность тишины  вместо 10 сделать 1 сек  (может быть потом через параметр задавать?)
...
Процедура УскоритьЗамедлитьФайлВидеоИДобавитьMp3(ПараметрыВидео, ИмяФайлВидео, ИмяФайлMP3, ДлительностьВидео, НоваяДлительностьВидео, ДлительностьMP3)
	...
	Если ПараметрыВидео.ЗаписьВидеоДелатьНаложениеTTS Тогда
		...
		ФайлТишины2 = СоздатьИмяФайлаТишины(
			ПараметрыВидео, 
			1 + Цел(ОпределитьДлительностьФайла(ПараметрыВидео, ИмяФайлаВидеоВременный)),  //+++ ЦВ_ПР 27.10.2025 Уменьшить длительность тишины  вместо 10 сделать 1 сек  (может быть потом через параметр задавать?)
	...
	Иначе
		// Значит нет озвучки голосом. Длительность тишины считаем по фактической длительности видео после масштабирования
		ФайлИмяФайлВидео = Новый Файл(ИмяФайлВидео);
		ФайлMp3 = СоздатьИмяФайлаТишины(
			ПараметрыВидео, 
			1 + Цел(ОпределитьДлительностьФайла(ПараметрыВидео, ИмяФайлаВидеоВременный)),
			ФайлИмяФайлВидео.ИмяБезРасширения + "_silence_0");
		ДобавитьMp3КВидео(ПараметрыВидео, ИмяФайлаВидеоВременный, ФайлMp3);
	КонецЕсли;	 
...
Процедура ДобавитьСубтитрыКВидео(ПараметрыВидео, ИмяФайлаШага, Шаг, ДлительностьMP3)
	...
	ЗТ.ЗаписатьСтроку("1"); 
	ЗТ.ЗаписатьСтроку(ТаймкодСубтитров(0.3) + " --> " + ТаймкодСубтитров(ДлительностьТитра + 0.3)); 
...
Процедура СобратьВидеоВставкуИзТекста(ПараметрыВидео, ИмяФайлаШага, Знач ТекстВставки, Знач ТекстДиктора, НомерСценария, НомерШага, ПараметрыГолоса, Шаг)
	...
	ФайлТишины2 = СоздатьИмяФайлаТишины(ПараметрыВидео, 4, СтрИмяФайла + "_silence_2"); //+++ ЦВ_ПР 27.10.2025 Уменьшить длительность тишины  вместо 20 сделать 4 сек  (может быть потом через параметр задавать?)
	ФайлMp3     = ОбъединитьФайлыMP3(ПараметрыВидео, ПараметрыВидео.ЗаписьВидеоКаталогДляВременныхФайлов + "\TTS", ФайлТишины1, ФайлMp3, ФайлТишины2);
...
Процедура СобратьВидеоВставкуИзКартинки(Шаг, ПараметрыВидео, ИмяФайлаШага, Знач ТекстВставки, ИмяКартинки, НомерСценария, НомерШага, ПараметрыГолоса, ДопПараметры = Неопределено)
	...
	ФайлФичи = Новый Файл(ПараметрыВидео.ПолныйПутьКФиче);
	Картинка = Ванесса.ПолучитьФайлКартинки(ИмяКартинки, ФайлФичи.Путь);
	ДелатьFadeIn = Истина;
...
Функция ЭтоВложенныйШагВнутриОбъединеннойГруппы(Шаг)
	Если Шаг.Свойство("ВнутриГруппыКотораяРаботаетКакОдинШаг")
		И (Ванесса.Объект.ЗаписьВидеоГруппаШаговКакШаг Или Ванесса.Объект.ТекстовыеИнструкцииГруппаШаговКакШаг) Тогда //+++ ЦВ_ПР 27.10.2025 Имеет смысл для настроек ..Группа..
		Если Шаг.ВнутриГруппыКотораяРаботаетКакОдинШаг Тогда
...
Процедура ОбработатьИнструкцииАвтоВидео(ПараметрыВидео)
	...
	Для каждого Сценарий Из МассивСценариевДляВыполнения Цикл
		...
		Для каждого Шаг Из Шаги Цикл
			...
			//+++ ЦВ_ПР 27.10.2025 Есть тег директивной остановки видеозаписи, пропустим такой шаг   
			Если Шаг.Свойство("RecordVideo") и Шаг.RecordVideo="Stop" 
				И НЕ (Шаг.Свойство("ДирективныйЗапускЗаписиВидео") и Шаг.ДирективныйЗапускЗаписиВидео) Тогда
				Продолжить;
		 	КонецЕсли;	 
		 	//--- ЦВ_ПР 27.10.2025 конец вставки	

			Если Шаг.Свойство("ИгнорироватьШагВАвтоинструкции") Тогда
				Если Шаг.ИгнорироватьШагВАвтоинструкции 
					И НЕ (Шаг.Свойство("ДирективныйЗапускЗаписиВидео") и Шаг.ДирективныйЗапускЗаписиВидео) Тогда //+++ ЦВ_ПР 22.10.2025 нужно учесть тег StartRecordVideo и не игнорировать шаг с тегом
					Продолжить;
				КонецЕсли;	 
			КонецЕсли;	 
			...
			Если Не ЗначениеЗаполнено(Текст) Тогда
				...
				Иначе	
					//Текст = Шаг.Имя; //так было
					//+++ ЦВ_ПР 22.10.2025 нужно учесть тег StartRecordVideo, который включает принудительно запись Видео и нежелательную озвучку названия шага в тексте отключим
...
Процедура ЗавершитьЗаписьИнструкцииВидео(ПараметрыВидео)
	...
	НомерСценария = -1;
	МассивСценариевДляВыполнения = ПараметрыВидео.МассивСценариевДляВыполнения;
	КешИДСценариевМассивСценариевДляВыполнения = КешИДСценариевМассивСценариевДляВыполнения(МассивСценариевДляВыполнения);
	Для каждого РезультатПрохожденияТестовСценария Из МассивРезультатПрохожденияТестовСценария Цикл
		...
		КешШаги = КешШаги(Шаги);
		
		НомерШага = -1;
		Для каждого ВыполненныйШаг Из РезультатПрохожденияТестовСценария.РезультатПрохожденияТестовШагов Цикл
			Если НЕ Ванесса.ИдетВыполнениеСценариев() Тогда
				Возврат;
			КонецЕсли;
			
			Шаг = КешШаги[ВыполненныйШаг.ИДСтроки];
			...
			Если ЭтоВложенныйШагВнутриОбъединеннойГруппы(Шаг)
				И НЕ (Шаг.Свойство("ИмяВидеоФайла") и ЗначениеЗаполнено(Шаг.ИмяВидеоФайла)) Тогда //+++ ЦВ_ПР 22.10.2025 Наличие Видео записи не пропускаем 
				Продолжить;
			КонецЕсли;	 
			...
			Если ВыполненныйШаг.Свойство("ИмяВидеоФайла") и ЗначениеЗаполнено(ВыполненныйШаг.ИмяВидеоФайла) тогда 
				ИмяФайлаШага = ВыполненныйШаг.ИмяВидеоФайла;	
			иначе     
			...
			Если Шаг.Свойство("ИгнорироватьШагВАвтоинструкции") Тогда
				Если Шаг.ИгнорироватьШагВАвтоинструкции 
					И НЕ (Шаг.Свойство("ДирективныйЗапускЗаписиВидео") и Шаг.ДирективныйЗапускЗаписиВидео) Тогда //+++ ЦВ_ПР 22.10.2025 нужно учесть тег StartRecordVideo
			...
			Если Ванесса.Объект.DebugLog Тогда
				Сообщить("Обработка фрагмента видео для шага с ид: " + Шаг.ИдСтроки);
			КонецЕсли;	
			...
			Если Шаг.Свойство("ИмяАудиоФайла") И ЗначениеЗаполнено(Шаг.ИмяАудиоФайла) тогда
			...
			Иначе
				...
				Иначе	
					// Значит нет озвучки - длительность шага считаем по времени чтения текста титра
					ТекстТитра = ?(ЗначениеЗаполнено(Шаг.ТекстИнструкция), Шаг.ТекстИнструкция, Шаг.Имя);
					СкоростьПроизношения = 15;
					Если ПараметрыВидео.Свойство("ЗаписьВидеоСкоростьПроизношенияТекстаБезОзвучки")
						И ЗначениеЗаполнено(ПараметрыВидео.ЗаписьВидеоСкоростьПроизношенияТекстаБезОзвучки) Тогда
						СкоростьПроизношения = ПараметрыВидео.ЗаписьВидеоСкоростьПроизношенияТекстаБезОзвучки;
					КонецЕсли;	 
					ДлительностьMP3 = Макс(2, Цел(СтрДлина(ТекстТитра) / СкоростьПроизношения) + 1);
				КонецЕсли;	 
	...
	Для каждого Сценарий Из МассивСценариевДляВыполнения Цикл
		...
		Для каждого Шаг Из Шаги Цикл
			...
			Или  СнипетыСубтитры.Найти(СнипетНрег) <> Неопределено
				...
				НомерШага = НомерШага + 1;
				Стр = "<p><b>%1</b>. %2</p>";     
				//+++ ЦВ_ПР 03.07.2025 И здесь нужно поменять ,3 на 5 для номера Шага (меняли ранее для имени файлов с целью корректной сортировки)
...
Функция ПолучитьФайлMP3ИзТекста(ПараметрыВидео, Текст, Знач ИмяФайла, ПараметрыГолоса = Неопределено)
	...
	Если ПараметрыВидео.ЗаписьВидеоКэшироватьФайлыTTS И ПараметрыВидео.Свойство("ДанныеКэшMp3") Тогда
		...
		Иначе	
			...
			ИначеЕсли ПараметрыГолоса.ТипОзвучки = 4 Тогда
								
				СтрокаПоиск = ПараметрыГолоса.Префикс + ПараметрыГолоса.Голос + РазделительСтроки 
							+ ПараметрыГолоса.Скорость + РазделительСтроки + ТекстДляMp3;
	...
	Для Сч = 1 По 3 Цикл
		...
		Попытка 
			...
			ИначеЕсли ПараметрыГолоса.Свойство("ТипОзвучки") И ПараметрыГолоса.ТипОзвучки = 4 Тогда
				
				// Озвучка от 1С
				
				ИмяMP3 = ПолучитьMP3ИзТекстаСервис1С(ПараметрыВидео,ТекстДляMp3, ИмяФайла, ПараметрыГолоса);
...
&НаСервереБезКонтекста
Функция СгенерироватьСервис1СНаСервере(Текст, Голос, ПараметрыСинтеза)
	
	ТекРаботаСРечью = Вычислить("РаботаСРечью");
	Возврат ТекРаботаСРечью.Синтезировать(Текст, НРег(Голос), ПараметрыСинтеза);
	
КонецФункции	

&НаКлиенте
Функция ПолучитьMP3ИзТекстаСервис1С(ПараметрыВидео,ТекстДляMp3, ИмяФайлаШага, ПараметрыГолоса)
	
	ПараметрыСинтеза = Новый Соответствие;
	ПараметрыСинтеза.Вставить("sample_rate", "22050");
	СтрокаСкоростьРечи = СтрЗаменить(Строка(Ванесса.Объект.ЗаписьВидео1ССкоростьРечи), ",", ".");
	ПараметрыСинтеза.Вставить("speed", СтрокаСкоростьРечи);
	
	ДвДанные = СгенерироватьСервис1СНаСервере(ТекстДляMp3, Ванесса.Объект.ЗаписьВидео1СГолос, ПараметрыСинтеза);
	
	ИмяWav = ПолучитьИмяВременногоФайла("wav");
	ДвДанные.Записать(ИмяWav);
	...
	КонецЕсли;	 
	ФинальноеИмяMp3 = КаталогФайлов + "\" + ИмяФайлаШага + ".mp3";
	Ванесса.ПереместитьФайлКомандаСистемы(ИмяMP3, ФинальноеИмяMp3);
	...
	Для каждого Сценарий Из МассивСценариевДляВыполнения Цикл
		...
		Для каждого Шаг Из Шаги Цикл
			...
			Если ПустаяСтрока(Шаг.ТекстИнструкция) ИЛИ Шаг.ТекстИнструкция = "*" Тогда
				ИмяMP3 = СоздатьИмяФайлаТишины(ПараметрыВидео, 1, ИмяФайла);  //+++ ЦВ_ПР 27.10.2025 Уменьшить длительность тишины  вместо 2 сделать 1 сек  (может быть потом через параметр задавать?)
			Иначе	
...
Функция ПолучитьПараметрыГолосаПоУмолчанию(ПараметрыВидео, ПараметрыГолоса = Неопределено)
	...
	ИначеЕсли ПараметрыВидео.ЗаписьВидеоТипОзвучкиTTS = 4 Тогда
		
		ПараметрыГолосаПоУмолчанию.Вставить("ТипОзвучки", 4);
		ПараметрыГолосаПоУмолчанию.Вставить("Префикс", "1c_");
		ПараметрыГолосаПоУмолчанию.Вставить("Голос", ПараметрыВидео.ЗаписьВидео1СГолос);
		ПараметрыГолосаПоУмолчанию.Вставить("Скорость", ПараметрыВидео.ЗаписьВидео1ССкоростьРечи);
...
Функция КаталогГдеЛежитКэш(ПараметрыВидео, БазовыйКаталог)
	...
	ИначеЕсли ПараметрыВидео.ЗаписьВидеоТипОзвучкиTTS = 4 Тогда
		//1С
		КаталогДанногоГолоса = БазовыйКаталог + "\1C_" + ПараметрыВидео.ЗаписьВидео1СГолос;
		Если НЕ Ванесса.ФайлСуществуетКомандаСистемы(КаталогДанногоГолоса) Тогда
			Ванесса.СоздатьКаталогКомандаСистемы(КаталогДанногоГолоса);
		КонецЕсли;	 
		
		ИмяКаталогПараметров = "Sp_"  + СтрЗаменить(Формат(ПараметрыВидео.ЗаписьВидео1ССкоростьРечи, "ЧГ=; ЧН=0"), ",", ".");
		Если НЕ Ванесса.ФайлСуществуетКомандаСистемы(КаталогДанногоГолоса + "\" + ИмяКаталогПараметров) Тогда
			Ванесса.СоздатьКаталогКомандаСистемы(КаталогДанногоГолоса + "\" + ИмяКаталогПараметров);
		КонецЕсли;
	Иначе	
...
Процедура ДобавитьMp3ШагаВКэш(ИмяMP3, Текст, ПараметрыВидео, ПараметрыГолоса)
	...
	ИначеЕсли ПараметрыГолоса.ТипОзвучки = 4 Тогда
		...
		ИмяКаталогПараметров = "Sp_"  + СтрЗаменить(Формат(ТекСкорость, "ЧГ=; ЧН=0"), ",", ".");
		...
		СтрокаПоиск = ПараметрыГолоса.Префикс + ТекГолос + РазделительСтроки 
					+ ТекСкорость + РазделительСтроки + Текст;
...
// Формирует таймкод субтитров вида ЧЧ:ММ:СС,ммм
&НаКлиенте
Функция ТаймкодСубтитров(Знач Секунды)
	Секунды = Макс(0, Секунды);
	ВсегоМиллисекунд = Окр(Секунды * 1000, 0);
	ВсегоСекунд = Цел(ВсегоМиллисекунд / 1000);
	Миллисекунды = ВсегоМиллисекунд - ВсегоСекунд * 1000;
	ВсегоМинут = Цел(ВсегоСекунд / 60);
	Возврат ДобавитьНулей(Цел(ВсегоМинут / 60), 2) + ":" + ДобавитьНулей(ВсегоМинут - Цел(ВсегоМинут / 60) * 60, 2)
		+ ":" + ДобавитьНулей(ВсегоСекунд - ВсегоМинут * 60, 2) + "," + ДобавитьНулей(Миллисекунды, 3);
КонецФункции	
...
Функция ПолучитьИмяФайлаВидеоШага(ПараметрыВидео)
	...
	Иначе	
		...
		Стр = "scen_" + ДобавитьНулей(ПараметрыВидео.ТекИД_СценарияВМассиве, 5) + "_step_" 
			+ Формат(ТекущаяУниверсальнаяДатаВМиллисекундах() , "ЧГ=; ЧН=0") + "_" + ДобавитьНулей(ПараметрыВидео.ТекИД_ШагаВМассиве, 5) +  "_main.mp4";
	КонецЕсли;	 
