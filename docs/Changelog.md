...
### AI
* Поддержка работы MCP для управления Vanessa Automation. #2516 Спасибо @DitriXNew
* Fix mcp extension location #2526 Спасибо @alkoleft
* Нужен MCP инструмент - база знаний с ответами на часто возникающие вопросы. #2537
* Нужен MCP инструмент - информация о часто используемых шагах. #2538
* Исправление в некоторых инструментах MCP #2545 Спасибо @d_bbnk
* Нужен MCP инструмент для работы с командным интерфейсом. #2554
* Нужен MCP инструмент для работы с таблицей клиентов тестирования: получить список, добавить строку, редактировать строку #2574
* Нужна возможность снимать скриншоты как в черно-белом варианте, так и в цветном. #2575
* Нужен инструмент для получения информации об окружении: ОС, ТекущаяДата(), ВерсияПлатформы, ВерсияVA #2577
* Нужен инструмент для голосового уведомления о выполнении задания. #2578
* Нужен инструмент для получения данных о значениях реквизитов и их типах из формы текущего документа или элемента справочника. #2579
* Нужен инструмент для получения данных из базы данных клиента тестирования. #2580
* Когда ИИ пытается найти элемент формы, которого нет, надо сразу возвращать кнопки, флаги, поля, которые есть. #2587
* Нужен инструмент для сохранения табличного документа в файл. #2588
* Нужен инструмент для получения данных текущего элемента формы. #2593
* Нужен инструмент - оператор #2594
* Нужен инструмент для выключения и выключения записи действий пользователя #2610
* Нужен инструмент для закрытия клиента тестирования #2611
* Возможность вызывать несколько действий в форме за один вызов инструмента. #2622
* MCP: подключение внешнего MCP-транспорта плагином (внешний фасад Мсп_Сервер) + сопутствующие фиксы #2614 Спасибо @DitriXNew
* Получение данных исследователя формы без моргания окна. #2646
* При получении константы возвращается сколько всего констант. Возможность получать константы по диапазону номеров. #2647
* При получении данных из справочников и документов возвращается общее число элементов. #2648
* При поиске элементов справочник возможность поиска по частичному совпадению наименования. #2649
* Добавить в документацию пример для написания тестов с использованием субагентов #2653
* Усиление синтакс-контроля при написании тестов ИИ #2654
* Нужна возможность читать данные ПВХ #2659
* Оптимизация модуля MCPVA #2660
* Нужен MCP инструмент - список процессов ОС с данными по ним (PID, командная строка, дата старта) через внешнюю компоненту VanessaExt #2712
* Нужен MCP инструмент activate_process_os - активизация окна процесса ОС по PID или по заголовку окна (аналог шагов <И я активизирую процесс "PID"> и <И я активизирую процесс по заголовку "Заголовок">) через внешнюю компоненту VanessaExt #2713
* Нужен MCP инструмент manage_uiautomation - работа с технологией UI Automation в любых приложениях: поиск элементов, чтение свойств и навигация по дереву, клики, ввод текста, сочетания клавиш, колесо мыши, курсор, визуальные эффекты (затемнение, рамка, эллипс, стрелка, подсказка-балун) через внешнюю компоненту VanessaExt #2714

### Новые возможности
* Поддержка работы MCP для управления Vanessa Automation. #2516
* Разделение основной формы на несколько модулей. Часть 1. #2661
* Буферизированная запись лога выполнения сценариев в текстовый файл: параметр РазмерБуфераЛогаВыполненияСценариев задаёт количество строк, накапливаемых перед записью в файл одним обращением. #2701
* Установка расширений в базу при запуске из командной строки: параметр InstallExtensionsAndClose (УстановитьРасширенияИЗакрыть) устанавливает расширения из файлов cfe, записывает результат установки в файл и закрывает сеанс Vanessa Automation. Тесты при этом не выполняются. #2702
* Настройка ОбъединятьПримерыСтруктурыСценарияВAllure (MergeScenarioOutlineExamplesInAllure): при включении все примеры одной структуры сценария выгружаются в отчет Allure как один тест-кейс — одинаковые fullName и testCaseId у всех примеров, значения параметров в parameters и name, уникальность прогонов в historyId. По умолчанию выключена, прежний формат выгрузки не меняется. По мотивам PR #2478 (issue #2477) #2715

### Новые шаги
* И я сохраняю ссылку на объект из текущего окна в переменную "ИмяПеременной" #2487
* И я сохраняю строковое представление ссылки на объект из текущего окна в переменную "ИмяПеременной" #2487
* И я открываю объект по ссылке "ИмяПеременной" (расширение) #2491
* И я открываю объект по строковому представлению ссылки "ИмяПеременной" (расширение) #2491
* Тогда появилось окно предупреждения системы #2591
* И я помечаю на удаление документы "ВидДокумента" содержащих комментарий "ТекстКомментария" (Расширение) #2623
* И я распаковываю архив "ПутьКФайлу" в каталог "ИмяКаталога" #2624
* И я отменяю проведение всех документов этого сценария, которые проводились, по их навигационным ссылкам (расширение) #2650
* И форма текущего окна имеет признак модифицированности #2676
* И форма текущего окна не имеет признак модифицированности #2676
* И я жду, что у формы текущего окна пропадёт признак модифицированности в течение 10 секунд #2676
* И я жду, что у формы текущего окна появится признак модифицированности в течение 10 секунд #2676
* И я запоминаю текущее окно как "ЗаголовокОкна" с запретом на пустое значение #2703
* Если появилось окно с заголовком из таблицы в течение 10 секунд Тогда #2638

### Пул реквесты
* Update GitHub Actions workflow for SonarCloud scan #2483 Спасибо @nixel2007
* Привести основную роль расширения VAExtension к стандарту 488 #2479 Спасибо @VladFrost
* Рефакторинг документации FAQ #2509 Спасибо @alei1180
* Fix for vi lang #2499 Спасибо @Kola110-dev
* fix issues_2505 #2506 Спасибо @ekh27
* Improve Swedish (sv) UI localization #2514 Спасибо @AlexeyPoltavets
* Обновление компоненты VanessaExt, версия 1.3.9.101 #2513 Спасибо @lintest
* Исправление нумерации параметров в английском переводе шагов #2507 Спасибо @Kola110-dev
* MCP в Vanessa Automation #2510 Спасибо @DitriXNew
* Удалён забытый отладочный код в ЗаполнитьКешСоответствияШаговПоискПоИмени #2527 Спасибо @OlegKopeykin
* Исправлена подсказка флага ЗапрещатьПерезаписыватьПеременные #2528 Спасибо @OlegKopeykin
* Обновлен GitHub Actions versions (checkout, setup-python, gh-pages) #2535 Спасибо @OlegKopeykin
* ci: nelonoel/branch-name → github.ref_name в Sonar-workflow #2546 Спасибо @OlegKopeykin
* chore: удалён пустой файл .gitmodules #2544 Спасибо @OlegKopeykin
* Удалены неиспользуемые процедуры ВыгрузитьНаборПеременных и ЗагрузитьНаборПеременных #2531 Спасибо @OlegKopeykin
* docs(tools): структурная переработка README для контрибьюторов #2536 Спасибо @OlegKopeykin
* ci: обновление mkdocs-material 5.0.0 → 9.x + кеш pip для gh-pages #2541 Спасибо @OlegKopeykin
* feat(Дымовые тесты): Возможность отфильтровать только отсутствующие тесты в фича-файлах #2522 Спасибо @Kyrales
* cleanup: удалена неиспользуемая серверная процедура УдалитьИзТаблицаИзвестныхStepDefinitionДанныеПоФайлуСервер #2543 Спасибо @OlegKopeykin
* leanup: удалена неиспользуемая процедура ЗаписатьДопПараметрыШагаТекстовойИнструкцииВЛог #2542 Спасибо @OlegKopeykin
* Удалить unreachable JRE 6 путь и неиспользуемый параметр ЗапускЧерезСкрипт #2532 Спасибо @OlegKopeykin
* Удалена неиспользуемая процедура ПолучитьФормуНастройкиРаботыСБраузером #2529 Спасибо @OlegKopeykin
* fix: устранён дубль 9 сценариев в MiddleCheck part1+part2 (баг парсера: теги «съедаются» блоком Переменные #2549 Спасибо @OlegKopeykin
* Исправить в сериализаторе выгрузку объектов со ссылкой #2547 Спасибо @1cgh
* perf: убран временный файл при формировании CSV-attachment в Allure 2 #2553 Спасибо @OlegKopeykin
* fix: Дедупликация тегов фичи и сценария в Allure 2 отчёте по значению #2552 Спасибо @OlegKopeykin
* Cleanup: убраны неиспользуемые места в Allure 2 pipeline #2551 Спасибо @OlegKopeykin
* Убран временный файл при формировании JSON отчёта Allure #2550 Спасибо @OlegKopeykin
* Обновление компоненты VanessaExt, версия 1.3.9.108 #2566 Спасибо @lintest
* cleanup: удалён мёртвый параметр СоответствиеГрупп из ЗаписатьВыполнениеШагаАллюр2 #2563 Спасибо @OlegKopeykin
* Обновление компоненты VanessaExt, версия 1.3.9.131 #2586 Спасибо @lintest
* fix: защита от Неопределено в ВремяНачала/ВремяОкончания при формировании отчётов (Allure 2, Cucumber JSON, XLS, TeamCity) #2567 Спасибо @OlegKopeykin
* fix: дать Chrome время на старт перед подключением к :9222 (flaky 'Снятие скриншота в браузере') #2583 Спасибо @OlegKopeykin
* cleanup: 8 dead vars в VanessaAutomation/Forms/РедакторТекста/Module.bsl #2582 Спасибо @OlegKopeykin
* cleanup: 10 dead vars в lib/Video/РаботаСВидео/Forms/Форма/Module.bsl #2581 Спасибо @OlegKopeykin
* сleanup: 25 dead local/module vars в lib/FeatureReader/.../ObjectModule.bsl #2573 Спасибо @OlegKopeykin
* сleanup: удалены 4 write-only модульные переменные в УправляемаяФорма #2570 Спасибо @OlegKopeykin
* cleanup: исправлены опечатки в именах функций Allure 2 #2569 Спасибо @OlegKopeykin
* perf(allure2): O(N²) → O(N) накопление CSV-attachment в DataTable #2568 Спасибо @OlegKopeykin
* Обновление редактора VAEditor, версия 1.3.7.6 #2592 Спасибо @OlegKopeykin @lintest
* сleanup: 13 dead local vars в VanessaAutomation/Ext/ObjectModule.bsl #2572 Спасибо @OlegKopeykin
* docs(allure2): cleanup фикстур Issue-34 — опечатки, cross-refs, осмысленные Функциональность #2571 Спасибо @OlegKopeykin
* cleanup: удалён мёртвый блок Если ШагВложенный <> Неопределено в ЗаписатьВыполнениеШагаАллюр2 #2565 Спасибо @OlegKopeykin
* cleanup: dead code в отчетах jUnit/CucumberJson/СППР/Allure2 (ObjectModule) #2601 Спасибо @OlegKopeykin
* cleanup: удаление повторных вставок одинаковых ключей в коллекции #2603 Спасибо @OlegKopeykin
* cleanup: Удаление мёртвой формы ГенераторТестов #2604 Спасибо @OlegKopeykin
* cleanup: Удаление мёртвой процедуры УстановитьИмяОбластиПеременных (РедакторТекста) #2605 Спасибо @OlegKopeykin
* fix: дефекты генерируемого EN-текста в генераторе дымовых тестов #2612 Спасибо @OlegKopeykin
* Обновление редактора VAEditor, версия 1.3.7.10 #2606 Спасибо @OlegKopeykin @lintest
* fix: опечатка ВидОбъектаРП у РегистрРасчета в генераторе дымовых тестов #2618 Спасибо @OlegKopeykin
* cleanup: удаление 10 мёртвых модульных переменных VanessaAutomation #2617 Спасибо @OlegKopeykin
* cleanup: удаление 6 мёртвых экспортных методов VA (6 коммитов) #2607 Спасибо @OlegKopeykin
* fix: ИнтернетПочта: исправления текстов регистраций шагов (опечатки, описания, аннотации) #2616 Спасибо @OlegKopeykin
* fix(отчёты): префикс имени сценария не добавлялся в jUnit и CucumberJson #2620 Спасибо @OlegKopeykin
* cleanup/perf(allure2): микро-чистка ObjectModule + O(1)-проверка исключаемых переменных #2619 Спасибо @OlegKopeykin
* cleanup: удалить артефакты story mapping (StoryMapper / checkushka.json) #2608 Спасибо @OlegKopeykin
* ИнтернетПочта: надёжность существующих шагов, чистка кода и документация #2625
* Update index.md #2627 Спасибо @SwDog
* cleanup(УправляемаяФорма): удаление 10 мёртвых локальных переменных #2632 Спасибо @OlegKopeykin
* Обновление компоненты VanessaExt, версия 1.3.9.133 #2633 Спасибо @lintest
* Исправление для вьетнамского языка, и номера параметров шагов в английском языке #2630 Спасибо @Kola110-dev
* Обновление компоненты VanessaExt, версия 1.3.9.135 #2640 Спасибо @steel-code-agent @lintest
* Обновление редактора VAEditor, версия 1.3.7.16 #2626 Спасибо @OlegKopeykin @lintest
* Обновление компоненты VanessaExt, версия 1.3.9.139- #2658 Спасибо @lintest
* Примеры использования в описаниях шагов: Цикл, ПереборМассива #2663 Спасибо @lyaginww
* Примеры использования в описаниях шагов: Условие и мелкие библиотеки #2665 Спасибо @lyaginww
* Примеры использования в описаниях шагов: библиотека VAExtension #2666 Спасибо @lyaginww
* Примеры использования в описаниях шагов: библиотека UIAutomation #2667 Спасибо @lyaginww
* Примеры использования в описаниях шагов: VanessaExtBrowser и Браузер #2668 Спасибо @lyaginww
* Примеры использования в описаниях шагов: РаботаСФайлами #2669 Спасибо @lyaginww
* Примеры использования в описаниях шагов: TestClients и РаботаСИнтерфейсомДополнительный #2670 Спасибо @lyaginww
* Примеры использования в описаниях шагов: SikuliX и VanessaExt #2671 Спасибо @lyaginww
* Примеры использования в описаниях шагов: ВстроенныйЯзык, ИнициаторДанных, ИнтернетПочта #2672 Спасибо @lyaginww
* Примеры в описаниях шагов оставшихся библиотек (94 шага) #2673 Спасибо @lyaginww
* Примеры в описаниях шагов: поля формы РаботаСИнтерфейсом (103) и оставшиеся шаги Цикл (3) #2674 Спасибо @lyaginww
* Примеры использования в описаниях шагов: таблицы РаботаСИнтерфейсом (157 шагов) #2678 Спасибо @lyaginww
* Примеры использования в описаниях шагов: остаток UI.Формы РаботаСИнтерфейсом (130 шагов) #2679 Спасибо @lyaginww
* Примеры использования в описаниях шагов: табличный документ, переменные, окна (85 шагов) #2680 Спасибо @lyaginww
* Примеры использования в описаниях шагов: командный интерфейс, прочее, всплывающие окна и файлы (72 шага) #2681 Спасибо @lyaginww
* Примеры использования в описаниях шагов: шаги без описания и ТипШага (38 шагов) #2682 Спасибо @lyaginww
* Обновление редактора VAEditor, версия 1.3.7.17 #2675 Спасибо @OlegKopeykin @lintest
* Вычитка описаний шагов: РаботаСИнтерфейсом, часть 1 #2687 Спасибо @lyaginww
* Вычитка описаний шагов: РаботаСИнтерфейсом, часть 2 #2688 Спасибо @lyaginww
* Вычитка описаний шагов: РаботаСИнтерфейсом, часть 3 #2689 Спасибо @lyaginww
* Вычитка описаний шагов: РаботаСИнтерфейсом, часть 4 #2690 Спасибо @lyaginww
* Вычитка описаний шагов: библиотека VAExtension #2691 Спасибо @lyaginww
* Вычитка описаний шагов: библиотека Условие #2692 Спасибо @lyaginww
* Вычитка описаний шагов: библиотеки UIAutomation и VanessaExtBrowser #2693 Спасибо @lyaginww
* Вычитка описаний шагов: библиотеки ЗапускПриложений и ИнициаторДанных #2694 Спасибо @lyaginww
* Вычитка описаний шагов: библиотеки Браузер и TestClients #2695 Спасибо @lyaginww
* Вычитка описаний шагов: РаботаСФайлами, РаботаСИнтерфейсомДополнительный, SikuliX #2696 Спасибо @lyaginww
* Вычитка описаний шагов: ИнтернетПочта, ВстроенныйЯзык, VanessaExt, ИнтерактивнаяСправка #2697 Спасибо @lyaginww
* Вычитка описаний шагов: Цикл, РаботаСТаблицами, ГенераторДанных и мелкие модули #2698 Спасибо @lyaginww
* Вычитка описаний шагов: семнадцать мелких модулей #2699 Спасибо @lyaginww
* Кнопка выравнивания разметки таблиц Gherkin в редакторе #2686 Спасибо @lyaginww
* Доработка формата выгрузки структуры сценария для Allure TestOps- #2478 Спасибо @LexxArh

### Разные доработки
* Сделать SleepOnServer в расширении VAExtension через HTTPСоединение. Чтобы не моргало черное окно при распроведении документа. #2488
* Переименовать некоторые шаги системы отката, чтобы по имени шага было понятнее что он делает #2489
* Не сохранять настройки пользователя в хранилище настроек, если идёт автоматическая сессия тестирования, запущенная из командной строки #2490
* Закрывать клиент тестирования до формирования отчетов о выполнении сценариев. Для экономии памяти #2494
* При прикладывании значений переменных к отчету Allure их нужно упорядочить по имени #2495
* Перенос методов VAEditor в отдельный модуль #2496
* Рефакторинг механизма сборки автоинструкций. Переход от дерева шагов к массиву выполненных сценариев. #2517
* Прикладывать к ошибкам сценарии, когда макетах есть оформление, а не только содержимое в виде массива данных. #2518
* Снижение потребления памяти, если упал шаг-условие #2525
* Шаги выполняющие пакетные команды конфигуратора должны выводить информацию в ЖР. #2539
* В JUnit не учитывается статус пропуска сценария установленный шагом И я останавливаю выполнение сценария "Skipped" #2523 Спасибо @1cgh
* Не учитывается каталог файлов исключений при генерации дымовых тестов из командной строки #2521 Спасибо @vsbronnikov
* Шаг 'И табличный документ "ИмяРеквизита" содержит значения' должен прикреплять текущее значение макета к отчету Allure #2555
* При поиске строк для локализации надо учитывать многострочные строки #2556
* Инструмент для вставки строк в макеты, которые обеспечивают локализацию #2557
* Инструмент для автоперевода макетов локализации #2559
* При запуске сборки в CI VA надо выводить данные о потреблении памяти процессами 1С #2584
* При проверке переключателей поддержать проверку значения в старом и новом формате - представление данных и отображаемый текст. #2621
* Исправление найденных опечаток #2634
* Убрать лишний пробел справа в имени окна, когда последний символ это * при работе с ИИ #2651
* При запуске клиента тестирования добавлена проверка наличия толстого клиента: если Vanessa запущена в тонком клиенте, а толстый клиент (1cv8/1cv8.exe) не установлен, выводится сообщение о необходимости его установить вместо бесполезной попытки запуска. #2704
* МодульРаботаСоСтроками переименован в МодульРаботаСоСтрокамиСлужебный. Модуль нужен только для вызова методов, когда менеджер тестирования не работает в режиме совместимости 8.2. #2705
* При получении состояния текущего элемента формы в виде шагов колонки таблицы выводятся в порядке, как они расположены в клиенте тестирования. #2706
* Длительность шага видео без озвучки теперь рассчитывается по времени чтения текста титра. Добавлена настройка "ЗаписьВидеоСкоростьПроизношенияТекстаБезОзвучки" (по умолчанию 15 символов в секунду). #2700
* Поиск окна по заголовку в шагах <И я активизирую процесс по заголовку> и <И я получаю данные процесса по заголовку окна> выполняется без учёта регистра #2716

### Исправление ошибок
* Дымовые тесты. Ошибка в проверке открытия формы #2497
* Окно с заголовком <$ЗаголовокФормы$> не найдено #2505 Спасибо @This1sMak @ekh27
* Утечка памяти при прохождении теста в 1.2.043.1 #2498 Спасибо @kakoytochelik
* При переносе кода в другой модуль сломалось две команды контекстного меню редактора #2540
* Выбор точного значения при установке фильтра на список #2533 Спасибо @ekh27
* Если установлена настройка повторного выполнения сценария и падал асинхронный шаг, то повторный запуск сценария всегда падал на этом шаге #2558
* В Vanessa Automation Single могла очищаться таблица известных шагов. #2560
* В Vanessa Automation Single могли не подгружаться снипеты из объектов метаданных. #2561
* Для флагов с типом число теперь используется корректная проверка на представление значения. #2595
* Исправление проблемы, когда не работала актуализация шага из-за шагов в попытке исключение. #2596
* Когда в сценарии есть несколько регистраций ошибки по данным исключения, то ошибка в формате json накапливала описание ошибки, а это неправильно #2609
* Затирание символа % в теле пароля пользователя. ВА 1.2.043.1 #2503 Спасибо @TatyanaOv
* Расширение не открывало внешний отчет #2628
* Найдена проблема при проверке представления переключателя в условии #2652
* Поведение шага "таблица содержит строки" и "таблица содержит строки по шаблону" приведено к одинаковому #2677
* Исправления поведения шага в мобильном клиенте #2707
* MCP get_table_data: исправлено падение «ПрочитатьJSON» при чтении справочников и ПВХ без реквизита «Наименование» (например, КБК); сброс «залипших» фильтров, владельца и дат между вызовами. VAExtension 1.32 #2708
* Исправление промтов из каталога examples #2709
* Исправление ошибки, которая иногда возникала при сравнении табличного документа #2710
* MCP: в уведомлениях прогресса теперь передаётся токен прогресса клиента из _meta.progressToken, а не идентификатор запроса JSON-RPC. Если клиент токен не передал, уведомление не отправляется. Из-за неверного токена Cursor рвал MCP-сессию после вызова run_scenario или load_features #2711
* MCP: при ошибках запуска сценариев (run_scenario, load_features) сбрасываются идентификатор задачи, токен прогресса и флаг оповещения MCP — после упавшего запуска при следующем выполнении сценариев не отправляются уведомления прогресса с устаревшим токеном #2711
* Ложное срабатывание проверки синтаксиса «Используется ключевое слово Иначе, повторно в одном условии» на два независимых условия Если-Иначе в одном сценарии. #2664
* В видеоинструкциях без озвучки: обрезались растянутые шаги, падала сборка субтитров из-за некорректного таймкода SRT, текст директивы [#[autodoc.text]] не попадал в субтитры и озвучку одиночных шагов. #2700
