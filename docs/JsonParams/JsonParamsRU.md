...
*  Дымовые тесты
      ...
      * **ДымовыеТестыТолькоИзмененныеОтносительноКонфигурацииПоставщика {SmokeTestsOnlyChangedRelativeToVendorConfiguration}**:
...
*  Прочие настройки выполнения сценариев
      ...
      * **ЗапрещатьПерезаписыватьПеременные {banoverwritingvariables}**:
         Если флаг установлен, то при попытке записи значения в ранее объявленную переменную во время выполнения шагов будет вызвано исключение, а не произойдёт молчаливая перезапись. Проверка применяется к обеим областям: обычному контексту (например, $Имя$) и сохраняемому контексту ($$Имя$$).
...
*  Запись событий в файл или консоль
      ...
      * **РазмерБуфераЛогаВыполненияСценариев {textlogbuffersize}**:
         Количество записей (шагов), которое накапливается в буфере перед записью в текстовый файл одним обращением к файлу.
         0 или 1 - запись как раньше, построчно.
         Значение больше 1 - накопление в буфере и запись порциями; буфер сбрасывается по заполнению и после окончания каждого сценария.
         При значении больше 1 запись в файл перестаёт быть полностью построчной: текущий шаг может появляться в файле с задержкой (до заполнения буфера).
...
*  Allure
   ...
   *  Основные настройки
         ...
         * **ПодставлятьВОтчетеAllureЗначенияПеременных {setvariablevaluesinstepsallurereport}**:
            ...
            Опцию нужно использовать осторожно, т.к. она требует дополнительных ресурсов для анализа, нужно ли подставлять переменные в шаг.
...
*  Видео
   ...
   *  Дополнительно
         ...
         * **ЗаписьВидеоСкоростьПроизношенияТекстаБезОзвучки {speedofspeechwithouttts}**:
            Скорость произношения текста (символов в секунду), используется для расчета длительности шага видео, если озвучка не делается.
