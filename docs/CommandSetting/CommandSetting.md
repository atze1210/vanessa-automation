...
| InstallExtensionsAndClose или УстановитьРасширенияИЗакрыть | Булево | Устанавливает расширения в базу из файлов .cfe и закрывает сеанс Vanessa Automation. Тесты при этом не выполняются. Несовместим с StartFeaturePlayer: если флаг передан, остальные параметры запуска игнорируются, кроме параметров ExtensionPath (ПутьКРасширению) и InstallExtensionsResult (РезультатУстановкиРасширений). Пользователь сеанса должен иметь право <АдминистрированиеРасширенийКонфигурации>. Подробнее см. раздел "Установка расширений при запуске из командной строки" ниже. |
| ExtensionPath или ПутьКРасширению | Строка | Пути к файлам расширений .cfe, разделённые символом "|". Используется только вместе с параметром InstallExtensionsAndClose (УстановитьРасширенияИЗакрыть). Символ "|" в самом пути не поддерживается. Запятая, пробелы и кириллица в путях допустимы. Точка с запятой в пути не поддерживается. |
| InstallExtensionsResult или РезультатУстановкиРасширений | Строка | Путь к файлу, в который записывается результат установки расширений: "0" — успешно, "5" — не получилось установить. Используется только вместе с параметром InstallExtensionsAndClose (УстановитьРасширенияИЗакрыть). Запускающей стороне рекомендуется проверять содержимое этого файла; отсутствие файла считается ошибкой установки. |
...
## Установка расширений при запуске из командной строки (InstallExtensionsAndClose)

Параметр **InstallExtensionsAndClose** (или **УстановитьРасширенияИЗакрыть**) запускает Vanessa Automation в режиме установки расширений: из файлов .cfe в базу устанавливаются расширения, результат записывается в файл результата, после чего сеанс закрывается. Тесты при этом не выполняются, остальные параметры запуска (в том числе StartFeaturePlayer и VAParams) игнорируются - не передавайте их вместе с этим флагом.

Пример строки запуска:

    "C:\Program Files (x86)\1cv8\8.3.23.1739\bin\1cv8c" /N"" /TestManager /DisableStartupDialogs /DisableStartupMessages /Out "C:\Temp\va-out.txt" /Execute "C:\Commons\rep\vanessa-automation\vanessa-automation.epf" /IBConnectionString "File=""c:\Bases\TestBaseTestManager"";" /C"InstallExtensionsAndClose;ПутьКРасширению=C:\Build\Ext1.cfe|C:\Build\Ext2.cfe;РезультатУстановкиРасширений=C:\Temp\result.txt"

Параметры режима:

- **InstallExtensionsAndClose** / **УстановитьРасширенияИЗакрыть** - флаг режима установки расширений (тесты не выполняются).
- **ExtensionPath** / **ПутьКРасширению** - пути к файлам расширений .cfe, разделённые символом "|". Символ "|" в самом пути не поддерживается (он разделитель). Запятая, пробелы и кириллица в путях допустимы. Точка с запятой в пути не поддерживается. Повторяющиеся пути устанавливаются один раз.
- **InstallExtensionsResult** / **РезультатУстановкиРасширений** - путь к файлу, в который записывается результат установки: "0" - успешно, "5" - не получилось установить. Передавайте фактический путь к файлу (значения "1", "0", "истина", "ложь" не поддерживаются).

Дополнительные требования и ограничения:

- Пользователь сеанса должен иметь право <АдминистрированиеРасширенийКонфигурации>.
- Передавайте абсолютные пути к файлам расширений и к файлу результата.
- Имя расширения берётся из самого файла cfe; если расширение с таким именем уже установлено в базе, оно обновляется.
- Установка выполняется в монопольном режиме - не запускайте этот режим параллельно с сеансами тестирования или другими сеансами в базе. Таймаут задания должен превышать время установки (включая ожидание монопольного режима до двух минут).
- Для установки нескольких расширений можно перечислить несколько путей через "|" - результат "0" будет записан только если все расширения установились. Детали ошибок установки записываются в журнал регистрации и в /Out файл сеанса.
- Файл результата записывается на клиенте после установки (монопольный режим к этому моменту уже снят).
- Символ "|" допустим в именах файлов на Linux - не используйте его в именах файлов расширений, если клиент тестирования работает на Linux.
