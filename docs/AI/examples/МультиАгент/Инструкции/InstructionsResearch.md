...
3. **Зафиксировать факт отсутствия** в файле `research/РезультатИсследования_<NN>_<КраткоеНазвание>.MD` в разделе «⛔ Блокировки и отсутствующие данные»:
...
- Ты должен проделать всё что требуется в задаче "вручную", т.е. с помощью только указанных инструментов (другие инструменты использовать нельзя; дополнительно разрешён инструмент чтения файла скриншота `Read`):
...
### 🚨 Анализ скриншотов при любых проблемах

Порядок «скриншот → `Read` → анализ → решение» и перечень обязательных точек скриншотов — в разделе «## 🖼️ Обязательное создание скриншотов (КРИТИЧНО ВАЖНО)» ниже (**единый источник истины**).

Если текстовый ответ инструмента не показывает ожидаемый результат или ты не можешь найти нужный элемент формы — действуй по этому алгоритму: **СРАЗУ** сделай скриншот (`get_window_screenshot_os`), **ПРОЧИТАЙ** его через `Read`, проанализируй, что реально на экране, и **только после анализа** принимай решение. Без скриншота ты работаешь вслепую — текстовые ответы инструментов могут не показывать невидимые, заблокированные или расположенные в другом месте элементы.
...
2. НЕМЕДЛЕННО вызвать Read для скриншота
...
✅ ПРАВИЛЬНО:
   ...
   Read → анализ → исправить действие → повторить
```

> ⚠️ **Разграничение ошибок:** технические сбои инструмента (таймаут, ошибка MCP и т.п.) — допустимы 1–2 повтора на уровне **отдельного действия**; логические/сценарные ошибки (нет элемента, неверные данные, блокировка сценария) — исправить действие, а при повторном неуспехе **остановиться и сообщить Управляющему Агенту** (не зацикливаться на попытках).
...
Шаг 3: Прочитать скриншот инструментом Read
...
├── Инструкции/                  # Инструкции для субагентов
...
2. Если каталог не существует — создайте его командой из корня проекта
   (убедитесь, что рядом лежат `ТестПлан.MD` и `ПараметрыКонфигурации.MD`):
   ```bash
   mkdir -p png
   ```
...
- Создавать каталоги (mkdir) вне корня проекта
...
4. **Записать в `РезультатИсследования_<NN>_<КраткоеНазвание>.MD`** в раздел «⛔ Блокировки и отсутствующие данные»:
...
- ✅ Обнаружил отсутствие → записал в `РезультатИсследования.MD` в раздел «⛔ Блокировки и отсутствующие данные».
