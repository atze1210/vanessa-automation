...
Для этого используй маркер `// Очистка данных` и/или реальные шаги удаления в группе `* Подготовка`.
...
> Комментарий — только пояснение к проверяющему шагу. Главное требование — наличие самих ПРОВЕРОК: для каждой проверки тестплана («Тогда») в feature должен быть содержательный проверяющий шаг — сравнение значения/факта (число, строка, дата, состояние, проводка и т.п.). Шаги навигации и ввода (например, «открылось окно», заполнение полей) проверками НЕ являются и их не заменяют: сценарий из одних таких шагов без проверок бракуется.
