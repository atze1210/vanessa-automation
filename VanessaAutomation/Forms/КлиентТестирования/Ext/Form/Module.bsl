...
// Проверяет является ли текущий клиент тестирования тонким, толстым или web клиентом
&НаКлиенте
Функция ЭтоТонкийТолстыйWebКлиент() Экспорт
	...
	Возврат ТекущиеДанные.ТипКлиента = "Тонкий"
		Или ТекущиеДанные.ТипКлиента = "Толстый"
		Или ТекущиеДанные.ТипКлиента = "Web";
	...
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		...
		Исключение
			...
			Иначе	 
				...
				СтрокаОшибки = СтрокаОшибки + Символы.ПС + Ванесса.Локализовать("Имя реквизита:") + " " + ИмяРеквизита;
				Если СчетчикПопыток < КоличествоПопыток Тогда
					Ванесса.Sleep(1);
				КонецЕсли;	 
			КонецЕсли;	 
...
Функция ВТаблицеЯВыбираюТекущуюСтроку(ИмяТЧ) Экспорт
	...
	Попытка
		//Проверка что в таблице есть строки. Выполняется только для тонкого, толстого и web клиента
		Если НЕ Ванесса.Версия851ИлиВыше И Ванесса.ЭтоТонкийТолстыйWebКлиент() Тогда
...
Процедура УпорядочитьЗначениеТаблицыФормыПоПорядкуКолонокЕслиЭтоВозможно(Таблица, ЗначениеТЧ) Экспорт
	...
	Попытка
		Если ЗначениеТЧ = Неопределено ИЛИ ЗначениеТЧ.Количество() = 0 Тогда
			Возврат;
		КонецЕсли;
		...
		Для Каждого Элем Из КолонкиТаблицы Цикл
			...
			ТекЗаголовок = НРег(НормализоватьЗаголовокКолонки(Элем.ТекстЗаголовка));
			Если ТекЗаголовок = "" ИЛИ ПроверкаНаДубльЗаголовка[ТекЗаголовок] <> Неопределено Тогда
				//Пустой или повторяющийся заголовок не участвует в упорядочивании.
				Продолжить;
			КонецЕсли;
		...
		Для Каждого СтрокаЗначениеТЧ Из ЗначениеТЧ Цикл
			...
			//ТекСтрока: нормализованный заголовок -> значение; СырыеКлючи: нормализованный заголовок -> исходный ключ
			ТекСтрока = Новый Соответствие;
			СырыеКлючи = Новый Соответствие;
			Для Каждого Элем Из СтрокаЗначениеТЧ Цикл
				Если ТипЗнч(Элем.Ключ) <> Тип("Строка") ИЛИ ПустаяСтрока(Элем.Ключ) Тогда
					//Нестроковый или пустой ключ не является заголовком колонки.
					Продолжить;
				КонецЕсли;
				Ключ = НРег(НормализоватьЗаголовокКолонки(Элем.Ключ));
				Если ТекСтрока.Получить(Ключ) <> Неопределено Тогда
					//Коллизия нормализации - непонятно как упорядочивать.
				...
				ТекСтрока.Вставить(Ключ, Элем.Значение);
				СырыеКлючи.Вставить(Ключ, Элем.Ключ);
			КонецЦикла;

			НоваяСтрока = Новый Массив;
			ИспользованныеЗаголовки = Новый Соответствие;
			Для Каждого Заголовок Из КолонкиВПравильномПорядке Цикл
				Ключ = НРег(НормализоватьЗаголовокКолонки(Заголовок));
				Если ТекСтрока.Получить(Ключ) = Неопределено Тогда
					Продолжить;
				КонецЕсли;

				ЗначениеЯчейки = Новый Структура;
				ЗначениеЯчейки.Вставить("Ключ", СырыеКлючи[Ключ]);
				ЗначениеЯчейки.Вставить("Значение", ТекСтрока[Ключ]);

				НоваяСтрока.Добавить(ЗначениеЯчейки);
				ИспользованныеЗаголовки.Вставить(Ключ, Истина);
			КонецЦикла;

			//дописываем колонки, которых нет в порядке таблицы, чтобы не терять данные
			Для Каждого КлючЗначение Из ТекСтрока Цикл
				Если ИспользованныеЗаголовки[КлючЗначение.Ключ] = Неопределено Тогда
					ЗначениеЯчейки = Новый Структура;
					ЗначениеЯчейки.Вставить("Ключ", СырыеКлючи[КлючЗначение.Ключ]);
					ЗначениеЯчейки.Вставить("Значение", КлючЗначение.Значение);
		...
		Если НоваяТаблица[0].Количество() > 0 Тогда
...
&НаКлиенте
Функция НормализоватьЗаголовокКолонки(Знач Заголовок)

	Если ТипЗнч(Заголовок) <> Тип("Строка") Тогда
		Возврат "";
	КонецЕсли;

	Заголовок = СтрЗаменить(Заголовок, Символы.ПС + Символы.ВК, " ");
	Заголовок = СтрЗаменить(Заголовок, Символы.ВК + Символы.ПС, " ");
	Заголовок = СтрЗаменить(Заголовок, Символы.ПС, " ");
	Заголовок = СтрЗаменить(Заголовок, Символы.ВК, " ");
				...
				Иначе
					...
					Для каждого СтрокаСтало Из ТЧстало Цикл
						...
						Для каждого Колонка Из СтрокаСтало Цикл
							...
							ЗначениеСтало = Колонка.Значение;
