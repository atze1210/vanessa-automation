...
Функция ИдСтрокиДереваПоНомеруСтрокиВВиджете(ИдВиджета, НомерСтроки) Экспорт
	ДанныеВиджета = ДанныеВиджетаПоИд(ИдВиджета);
...
Процедура ОтрисоватьБрейкпоинтыVanessaEditor() Экспорт
	...
	Если ЭтоВкладкаEdit(Ванесса.VanessaTabs.current) Тогда
	...
	Если БрейкпоинтыVanessaEditor <> Неопределено Тогда
		Для Каждого Элем Из БрейкпоинтыVanessaEditor Цикл
			Если Элем.Включен Тогда
				...
				Иначе
					ДанныеВиджета = ДанныеВиджетаПоИд(Элем.ИдВиджета);
...
Процедура ПриСменеЗакладкиРедактора(Вкладка) Экспорт
	...
	Ванесса.Элементы.ГруппаVanessaEditorПереходКРазличиямВФайлах.Видимость = ЭтоВкладкаDiff(Вкладка);

	Если ЭтоВкладкаEdit(Вкладка) Тогда
...
Процедура ПоказатьСтраницуWelcome(НедавноЗагруженныеФичи) Экспорт
	...
	МассивКомандЗапуска.Добавить(Новый Структура("event,href,gliph,name", "WELCOME_LAUNCH", "Ванесса.ЗагрузитьОднуФичу()", "go-to-file", Ванесса.Локализовать("Открыть feature файл…")));
	МассивКомандЗапуска.Добавить(Новый Структура("event,href,gliph,name", "WELCOME_LAUNCH", "Ванесса.ЗагрузитьФичиИзКаталога()", "folder-opened", Ванесса.Локализовать("Открыть папку…")));
	...
	МассивДляОбучения.Добавить(Новый Структура("event,href,gliph,name", "WELCOME_LAUNCH", "VanessaEditorОткрытьСписокУроков()", "mortar-board", Ванесса.Локализовать("Открыть список уроков")));
...
Процедура VanessaEditorПоказатьСписокТочекОстанова(Команда) Экспорт
	Если Ванесса.VanessaTabs = Неопределено ИЛИ Ванесса.VanessaTabs.current = Неопределено ИЛИ НЕ ЭтоВкладкаEdit(Ванесса.VanessaTabs.current) Тогда
		Возврат;
	КонецЕсли;	 
...
Процедура СохранитьДанныеОбОшибкеVanessaEditor(ТекШаг, ДанныеПоОшибке, ТекстОшибки, СтрокаШага) Экспорт
	...
	ДанныеОшибки.Вставить("НомерСтрокиВФиче", ТекШаг.НомерСтрокиВФиче);
	ИмяФичи = Ванесса.ПолучитьИмяФичиИзПолногоПути(СтрокаШага.ПолныйПуть);
	ДанныеОшибки.Вставить("ИмяФичи", ИмяФичи);
...
Процедура VanessaEditorOnReceiveEventHandler(Event, Arg) Экспорт
	...
	ИначеЕсли Event = "PRESS_ESCAPE" Тогда
		Если (НЕ Ванесса.ЗначениеДополнительногоПараметра("ВЭтомСеансеИдётОбучение") = Истина)
			...
			И (ВозможенЗапускСценариевИзТекущейВкладки() ИЛИ ЭтоВкладкаDiff(Ванесса.VanessaTabs.current)) Тогда
...
&НаКлиенте
Процедура VanessaEditorОткрытьСписокУроков()
	Ванесса.VanessaEditorОткрытьСписокУроковЭкспорт();
КонецПроцедуры

&НаКлиенте
Процедура ДемонстрацияРаботыСправкиПоЭлементу()
	Ванесса.ДемонстрацияРаботыСправкиПоЭлементу();
КонецПроцедуры
...
Процедура ПерейтиВVanessaEditorПродолжение2() Экспорт
	...
	Если НЕ ПустаяСтрока(Ванесса.Объект.КаталогФич) Тогда
		...
		Если ТекФича = Неопределено ИЛИ ЗагруженКаталог ИЛИ НРег(ТекФича) <> НРег(Ванесса.Объект.КаталогФич) ИЛИ НЕ ЕстьВкладкаСДаннойФичей(ТекФича) ИЛИ НадоБезусловноПерезагрузить Тогда
...
Процедура ЗаменитьТаблицуGherkinПослеРедактирования(МассивСтрокТаблицы, НомерПервойСтроки = Неопределено) Экспорт
	...
	Если Ванесса.Объект.ИспользоватьРедакторVanessaEditor И Ванесса.Элементы.ФункциональностьЗакладки.ТекущаяСтраница = Ванесса.Элементы.ГруппаЗапускТестов Тогда
		...
		МаксСтр = 0;
		ДлинаМаксСтр = 0;
		ТаблицаGherkinИзVanessaEditorНачинаяСоСтроки(ТекущаяСтрокаVanessaEditor, МинСтр, МаксСтр, ДлинаМаксСтр);
...
Процедура ПолучитьШагПроверкиТекущегоОкна(Команда) Экспорт
	...
	Результат = "Тогда открылось окно '" + Ванесса.ЭкранироватьСпецСимволыДляЗначенияШагаКлиент(АктивноеОкно.Заголовок, Ложь) + "'";
...
// Проверяет, есть ли звёздочка (*) в строках таблицы Gherkin.
&НаКлиенте
Функция ВЗначенияхТаблицыGherkinЕстьЗвездочка(МассивСтрок) Экспорт
...
Процедура СделатьВставкуВиджетаСПодчиненнымиСтрокамиVanessaEditor(СтрокаДерева, ТекУровень, СтрокиПодсценариевВиджетов)
	...
	ДобавитьДанныеВиджета(ИДВиджета, ДанныеВиджета);	
КонецПроцедуры 
...
Функция СтруктураПараметровДляПолученияДанныхШагов(Язык)
	...
	СтруктураПараметров.Вставить("ДанныеДляВставкиГиперссылокВОписаниеШагов", ДанныеДляВставкиГиперссылокВОписаниеШагов());
...
&НаКлиенте
Процедура ОбновитьТекущуюСтрокуРедактораПоТекущейСтрокеДереваОбработчик() Экспорт
...
#Область ВкладкиИБрейкпоинты

// Проверяет, что вкладка является вкладкой, показывающей различие в файлах.
&НаКлиенте
...
Функция ВозможенЗапускСценариевИзТекущейВкладки() Экспорт
	...
	Если Ванесса.VanessaTabs.current = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	Если ЭтоВкладкаDiff(Ванесса.VanessaTabs.current) Тогда
		Возврат Ложь;
	КонецЕсли;

	Если НЕ ЭтоВкладкаEdit(Ванесса.VanessaTabs.current) Тогда
		Возврат Ложь;
	КонецЕсли;
...
// Проверяет, есть ли уже вкладка с данной фичей.
&НаКлиенте
Функция ЕстьВкладкаСДаннойФичей(ИмяФичи) Экспорт
	Если Ванесса.VanessaTabs = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;

	Для Сч = 0 По Ванесса.VanessaTabs.tabStack.length - 1 Цикл
		Если Ванесса.УниверсальноеПолноеИмяФайлаКлиент(Ванесса.VanessaTabs.tabStack[Сч].filename, Истина) = Ванесса.УниверсальноеПолноеИмяФайлаКлиент(ИмяФичи, Истина) Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
...
// Добавляет точку останова по полному пути.
&НаКлиенте
Процедура ДобавитьБрейкПоинт(ПолныйПуть) Экспорт
	Ванесса.Брейкпоинты().Добавить(ПолныйПуть);
	Если НЕ Ванесса.Элементы.ДеревоТестов.ПодчиненныеЭлементы.ДеревоТестовБрейкпоинт.Видимость  Тогда
		Ванесса.Элементы.ДеревоТестов.ПодчиненныеЭлементы.ДеревоТестовБрейкпоинт.Видимость = Истина;
	КонецЕсли;
КонецПроцедуры

// Удаляет точку останова по полному пути.
&НаКлиенте
Процедура УдалитьБрейкПоинт(ПолныйПуть) Экспорт
	ИД = Ванесса.Брейкпоинты().Найти(ПолныйПуть);
	Если ИД <> Неопределено Тогда
		Ванесса.Брейкпоинты().Удалить(ИД);
	КонецЕсли;

	Если Ванесса.Брейкпоинты().Количество() = 0 Тогда
		Если Ванесса.Элементы.ДеревоТестов.ПодчиненныеЭлементы.ДеревоТестовБрейкпоинт.Видимость Тогда
			Ванесса.Элементы.ДеревоТестов.ПодчиненныеЭлементы.ДеревоТестовБрейкпоинт.Видимость = Ложь;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры

// Рекурсивно снимает пометку брейкпоинта в строках дерева.
&НаКлиенте
Процедура СнятьПометкуБрейкпоинтаРекурсивно(СтрокиДерева) Экспорт
...
Процедура УстановитьТемуРедактора(Тема) Экспорт
	...
	Ванесса.Объект.ТемаРедактораVanessaEditor = Тема;
	ТемаРедактораVanessaEditorПриИзменении(Неопределено);

КонецПроцедуры

// Сохраняет текст файла вкладки и закрывает вкладку.
&НаКлиенте
Процедура СохранитьТекстФайлаВкладкиИЗакрытьВкладку(Data, filename = "") Экспорт
	...
	Состояние(Ванесса.Локализовать("Сохранение файла."), , Data.title, БиблиотекаКартинок.ДиалогИнформация);
...
// Фомирует данные для подставновки гиперссылок в описание шагов
&НаКлиенте
Функция ДанныеДляВставкиГиперссылокВОписаниеШагов() Экспорт
	
	Массив = Новый Массив;
	
	Структура = Новый Структура;
	Структура.Вставить("СтрокаПоиска", "VAExtension");
	Структура.Вставить("НРегСтрокаПоиска", "vaextension");
	Структура.Вставить("Гиперссылка", "https://github.com/Pr-Mex/vanessa-automation/blob/develop/docs/VAExtension/VAExtension.md");
...
Процедура ПолучитьТекстШагаИзТекстаФичиЕслиОнТамЗадан(Текст, Шаг, Сценарий) Экспорт
	Ванесса.МодульАвтоинструкции().ПолучитьТекстШагаИзТекстаФичиЕслиОнТамЗадан(Текст, Шаг, Сценарий);
КонецПроцедуры
...
Функция ВывестиВСообщениеТекстОшибки(ДанныеОшибки, ТекстОшибкиTestClientБезФорматирования = "") Экспорт
	Возврат Ванесса.МодульКлиентТестирования().ВывестиВСообщениеТекстОшибки(ДанныеОшибки, ТекстОшибкиTestClientБезФорматирования);
КонецФункции

// ТекстОшибкиЧерезДанныеОкнаОшибки
&НаКлиенте
Функция ТекстОшибкиЧерезДанныеОкнаОшибки() Экспорт
	Возврат Ванесса.МодульКлиентТестирования().ТекстОшибкиЧерезДанныеОкнаОшибки();
КонецФункции

// ПоказатьТекстОшибки
&НаКлиенте
Процедура ПоказатьТекстОшибки(ТекстОшибки, ПроизвольныеЗначения, НомерСтрокиДерева, ДопПараметры = Неопределено) Экспорт
	Ванесса.МодульКлиентТестирования().ПоказатьТекстОшибки(ТекстОшибки, ПроизвольныеЗначения, НомерСтрокиДерева, ДопПараметры);
КонецПроцедуры
// ЯзыкФичиИзТекста
&НаКлиенте
Функция ЯзыкФичиИзТекста(Текст) Экспорт
	Возврат Ванесса.МодульПарсерФич().ЯзыкФичиИзТекста(Текст);
КонецФункции
...
Процедура ОпределитьГраницыТаблицыGherkin(НачалоСтроки, НачалоКолонки, КонецСтроки, КонецКолонки, ВерхняяГраница, НижняяГраница) Экспорт
	Ванесса.Элементы.СгенерированныйСценарий.ПолучитьГраницыВыделения(НачалоСтроки, НачалоКолонки, КонецСтроки, КонецКолонки);
...
&НаКлиенте
Функция ПолучитьПервуюСтроку(Знач Стр) Экспорт
	Если Найти(Стр, Символы.ПС) = 0 Тогда
		Возврат Стр;
	КонецЕсли;

	МассивСтрок = Ванесса._СтрРазделить(Стр, Символы.ПС, Ложь);
