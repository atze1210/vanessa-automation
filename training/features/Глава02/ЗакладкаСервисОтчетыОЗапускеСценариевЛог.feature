...
Сценарий: Запись событий выполнения сценариев в файл
	...
	* Данный флаг определяет нужно ли записывать дополнительную информацию по статистике выполнения тестов.
		И я делаю подсветку нескольких элементов VA с их заголовком "Запись дополнительной информации" UI Automation
			| 'Имя'                             |
	...
	* Данный флаг определяет нужно ли скрывать пароль для подключения клиента тестирования в логе Vanessa Automation
		И я делаю подсветку нескольких элементов VA с их заголовком "Маскировать пароль в логе" UI Automation
			| 'Имя'                             |
			| 'МаскироватьПарольВЛоге' |

	* Этот параметр задаёт размер буфера: сколько записей (шагов) накапливается перед записью в файл одним обращением. Если указано 0 или 1, запись будет происходить построчно.
		И я делаю подсветку нескольких элементов VA с их заголовком "Размер буфера записи лога" UI Automation
			| 'Имя'                                 |
			| 'РазмерБуфераЛогаВыполненияСценариев' |
